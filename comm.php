...
if ($section == "write") {
	if ($myUser->uknsperr == 1) {
		...
		if (($sent == 1) && !$errormsg) {
			...
			$myComm->addkn(strip_tags($subject, "<b><i>"), strip_tags($message, "<b><i>"), $off);
...
if ($section == "allywrite") {
	...
	if (($sent == 1) && !$errormsg) {
		...
		$message = addslashes($message);
		$subject = addslashes($subject);
		$myComm->addallykn(strip_tags($subject, "<b><i>"), strip_tags($message, "<b><i>"));
		$return['msg'] = "Beitrag hinzugefügt";
		$section = "allykn";
	}
}
...
if ($section == "edit") {
	...
	if (($sent == 1) && !$errormsg) {
		$subject = $subject ?? '';
		$return = $myComm->editkn(strip_tags($subject, "<b><i>"), strip_tags($message, "<b><i>"), $id);
		$section = "kn";
	}
}

$lzcount = $myComm->getlzcount();
$lzcount = ($lzcount == 0) ? '' : "<a href=\"main.php?page=comm&section=kn&mark={$myUser->ukn_lz}\">Beiträge ab Lesezeichen</a>: {$lzcount}<br>";

$allylzcount = $myComm->getallylzcount();
$allylzcount = ($allylzcount == 0) ? '' : "<a href=\"main.php?page=comm&section=allykn&mark={$myUser->ukn_allylz}\">Beiträge ab Lesezeichen</a>: {$allylzcount}<br>";

if ($action == "knanlr") {
	$myDB->query("UPDATE stu_user SET knanl=0 WHERE id=" . (int)$user);
	$myUser->uknanl = 0;
}

if ($myUser->uknanl == 1 && ($section == "kn" || $section == "write")) {
	$section = "knanl";
}

if (!$section) {
	if ($myUser->ually > 0) {
		$ally = '<td valign="top" width="30%">
            <table width="100%" align="center" cellspacing="1" cellpadding="1" bgcolor="#262323">
                <tr>
	...
	if ($myUser->ustatus == 8) {
		echo "<br><a href=\"?page=comm&section=viewmsgs\">PM-Archiv</a>";
	}
	echo '</td></tr>
                    </table>
                </td>';
	echo !$ally ? "<td width=\"40%\">&nbsp;</td>" : "<td width=\"5%\">&nbsp;</td>" . $ally;
	echo '</tr>
        </table><br>
        <table width="100%" align="center" cellspacing="1" cellpadding="1">
            <tr>
	...
	if ($msg == 0) {
		$kn = "<table align=center cellpadding=1 cellspacin=1 bgcolor=#262323><tr><td class=tdmainobg align=center>Keine Beiträge vorhanden</td></tr></table>";
	} else {
	...
	if ($msg == 0) {
		echo "<table cellspacing=1 cellpadding=1 bgcolor=#262323><tr><td class=tdmain><strong>Meldung</strong></td></tr><tr><td class=tdmainobg align=center>Keinen Beitrag gefunden</td></tr></table><br>";
	} else {
		if ($vt == 0) {
			if ($msg['status'] == 9) {
			...
			echo "<table width=800 border=1 cellspacing=2 cellpadding=2 align=center bordercolor=#000000 bgcolor=#262323>
				...
				<tr>
				    <td width=610 class=tdmainobg valign=top>" . $rasse . $msg['user'] . " " . $id . " <a href=main.php?page=comm&section=writepm&recipient=" . $msg['user_id'] . " onMouseOver=document.msg0.src='" . $grafik . "/buttons/msg2.gif' onMouseOut=document.msg0.src='" . $grafik . "/buttons/msg1.gif'><img src=" . $grafik . "/buttons/msg1.gif name=msg0 border=0 title='private Nachricht schreiben'></a> <a href=main.php?page=hally&section=sinfo&id=" . $msg['user_id'] . " onMouseOver=document.sinfo0.src='" . $grafik . "/buttons/info2.gif' onMouseOut=document.sinfo0.src='" . $grafik . "/buttons/info1.gif'><img src='" . $grafik . "/buttons/info1.gif' name=sinfo0 border=0 title='Spielerprofil'></a> <a href=main.php?page=comm&section=kn&setmark=" . $msg['id'] . " onMouseOver=document.lese0.src='" . $grafik . "/buttons/lese2.gif' onMouseOut=document.lese0.src='" . $grafik . "/buttons/lese1.gif'><img src=" . $grafik . "/buttons/lese1.gif name=lese0 border=0 title='Lesezeichen setzen'></a></td>
	...
	<tr>
		<td valign=top>";
	if (mysql_num_rows($res) == 0) {
		echo "<table width=600 align=center bgcolor=#26232><tr><td align=center class=tdmain>Keine Nachrichten vorhanden</td></tr></table>";
	} else {
		while ($msg = mysql_fetch_assoc($res)) {
	...
	$new = ($cated['new'] > 0) ? "<font color=Red>" . $cated['new'] . "</font>" : 0;
	...
	if (mysql_num_rows($rmsg) == 0) {
		$kn = "<table align=center cellspacing=1 cellpadding=1 bgcolor=#262323><tr><td class=tdmainobg align=center>Keine Beiträge vorhanden</td></tr></table>";
	} else {
		while ($msg = mysql_fetch_assoc($rmsg)) {
			$delbtn = "";
			if ($dm == 1) $delbtn = " <a href=main.php?page=comm&section=allykn&del=" . $msg['id'] . " onMouseOver=cp('del" . $i . "','buttons/x2') onMouseOut=cp('del" . $i . "','buttons/x1')><img src=" . $grafik . "/buttons/x1.gif name=del" . $i . " border=0 title='Löschen'></a>";
			$id = ($msg['status'] == 9) ? "<strong>NPC</strong>" : "(" . $msg['user_id'] . ")";
			$kngfx = ($msg['picture'] != "") ? $msg['picture'] : $grafik . "/rassen/" . $msg['rasse'] . "kn.gif";
			$kn .= "<table width=800 border=1 cellspacing=2 cellpadding=2 align=center bordercolor=#000000 bgcolor=#262323>
			<tr>
	...
	<tr>
		<td class=tdmainobg colspan=2>
			<input type=radio name=type value=user CHECKED> User 
			<input type=radio name=type value=ally> Allianz 
			...
			<a href=?page=comm&section=editcontacts&action=dellist onmouseover=cp('dela','buttons/x2') onmouseout=cp('dela','buttons/x1')>
				<img src=" . $grafik . "/buttons/x1.gif title='Liste leeren' name=dela border=0>
			</a>
		</td>
	</tr>";
	$list = $myComm->getcontacts($user);
	if ($list == 0) {
		echo "<tr><td class=tdmainobg align=center colspan=2>Keine Kontakte vorhanden</td></tr>";
	} else {
		for ($i = 0; $i < count($list); $i++) {
			...
			echo "<tr>
				<td width=5% class=tdmainobg>
					<a href=main.php?page=comm&section=editcontacts&action=del&recipient=" . $list[$i]['recipient'] . " onMouseOver=document.del" . $i . ".src='" . $grafik . "/buttons/x2.gif' onMouseOut=document.del" . $i . ".src='" . $grafik . "/buttons/x1.gif'>
						<img src=" . $grafik . "/buttons/x1.gif name=del" . $i . " border=0 title='Löschen'>
					</a>
				</td>
				<td class=tdmainobg width=95%>" . stripslashes($myUser->getfield("user", $list[$i]['recipient'])) . " (ID: " . $list[$i]['recipient'] . " - Status: " . $status . ")</td>
			</tr>";
		}
	}
	echo "</table><br>";
	...
	if ($myUser->ustatus != 8) {
		echo "<table width=200 cellspacing=1 cellpadding=1 bgcolor=#262323><tr><td class=tdmain><strong>Meldung</strong></td></tr><tr><td class=tdmainobg>Zugriff verweigert!</td></tr></table><br>";
	} else {
		if (!$action) {
			echo "<table width=200 cellspacing=1 cellpadding=1 bgcolor=#262323>
			<tr>
				<td class=tdmainobg>
					<a href=?page=comm&section=viewmsgs&action=searchmsg>PMs durchsuchen</a><br>
					<a href=?page=comm&section=viewmsgs&action=usermsg>PMs eines Users ansehen</a>
				</td>
			</tr>";
			...
			if ($search) {
				...
				<tr><td class=tdmainobg>Die Suche nach <b>" . htmlspecialchars($search) . "</b> ergab ";
				echo ($msg == 0 ? 0 : count($msg)) . " Treffer</td></tr></table><br>";
			} else {
				echo "<table width=300 cellspacing=1 cellpadding=1 bgcolor=#262323>
				<form action=main.php method=post>
					<input type=hidden name=page value=comm>
					...
					<tr>
						<td class=tdmainobg>Suchbegriff eingeben: <input type=text class=text size=10 name=search> <input type=submit class=button value=Search></td>
					</tr>
				</form></table>";
			}
			if ($msg != 0) {
				for ($i = 0; $i < count($msg); $i++) {
					...
					foreach ($test as $word) {
						$newmsg = str_replace($word, "<b>" . htmlspecialchars($word) . "</b>", $newmsg);
					}
					echo $newmsg . "</td></tr></table>";
				}
			}
		} elseif ($action == "usermsg") {
			if ($search) {
				$msg = $myComm->getpmsbyuserid($search);
				echo "<tr><td class=tdmainobg>Nachrichten von UserID <b>" . (int)$search . "</b> (" . htmlspecialchars($myUser->getfield("user", $search)) . "): ";
				echo ($msg == 0 ? 0 : count($msg)) . "</td></tr></table><br>";
			} else {
				echo "<table width=300 cellspacing=1 cellpadding=1 bgcolor=#262323>
				<form action=main.php method=post>
					<input type=hidden name=page value=comm>
					...
					<tr>
						<td class=tdmainobg>UserId eingeben: <input type=text class=text size=10 name=search> <input type=submit class=button value=Anzeige></td>
					</tr>
				</form></table>";
			}
			if ($msg != 0) {
	...
	if ($msg == 0) {
		...
		while ($i < count($msg)) {
			$npc = ($msg[$i]['status'] == 9) ? "<strong>NPC</strong> " : "(" . $msg[$i]['recipient'] . ")";
			echo "<table width=600 border=1 cellspacing=2 cellpadding=2 align=center bordercolor=#000000 bgcolor=#262323>
				<tr>
	...
	echo "<tr><td class=tdmainobg>
		<a href=main.php?page=comm&section=outpm&begin=" . $vor . " onMouseOver=document.vora.src='" . $grafik . "/buttons/b_from2.gif' onMouseOut=document.vora.src='" . $grafik . "/buttons/b_from1.gif'><img src='" . $grafik . "/buttons/b_from1.gif' name=vora border=0 title='vorblättern'></a>
	...
	<table cellspacing=1 cellpadding=1 bgcolor=#262323>
		...
		<tr><td class=tdmainobg>
			Das Kommunikationsnetzwerk stellt eine der wichtigsten Funktionen im Spiel dar. Die meisten wichtigen Ereignisse spielen sich hier ab.
			...
			Alles, was außerhalb der RPG-Ebene stattfindet, ist im KN fehl am Platz. Bugberichte, Offtopic etc. gehören ins Forum.<br><br>
			...
			4. Respekt<br>
			Jedem Spieler ist ein Grundmaß an Respekt entgegenzubringen. Daher sind z. B. <b>persönliche</b> Beleidigungen zu unterlassen.<br><br>
			...
			Inoffiziell ist für alles, das nicht öffentlich sichtbar ist. Dies können z. B. Ereignisse innerhalb eines RPGs sein, die auf Planeten oder Schiffen stattfinden und Hintergründe erläutern.<br><br>

			Dieser Leitfaden erscheint automatisch beim ersten Aufruf des Kommunikationsnetzwerks. Danach ist er per Link im Bildschirm Kommunikation erreichbar.
		</td></tr>
	</table>";
	if ($myUser->uknanl == 1) {
		echo "<br><form action=main.php method=post>
			<input type=hidden name=page value=comm>
			<input type=hidden name=action value=knanlr>
			<input type=submit value=Bestätigung class=button>
		</form>";
	}
}