...
class colony {
	...
	function buildonfield($fieldId,$buildingId,$colId,$userId) {
		...
		if (($data['energie'] - $building['eps_cost']) < 0) {
			$return['msg'] = "Zum Bau werden ".$building['eps_cost']." Energie ben�tigt - Vorhanden ist aber nur $return['msg']e];
			$return['code'] = 0;
			return $return;
		}
		global $myUser;
	...
	function deactivateBuilding($fieldId,$colId,$userId) {
		...
		if ($field['aktiv'] == 0) {
			$return['msg'] = "Das Geb�ude ist bereits deaktiviert";
			return $return;
		}
		if ($build['bev_pro'] > 0) {
			if ($build['bev_pro'] + $data['bev_used'] <= $data['max_bev']) mysql_query("UPDATE stu_colonies SET max_bev=max_bev-".$build['bev_pro']." WHERE id='".$data['id']."'",$this->dblink);
				...
				$return['msg'] = "Das Geb�ude konnte nicht deaktiviert werden da von dem Wohnraum noch Siedler arbeiten";
				$return['code'] = 0;
				return $return;
			}
		}
	...
	function renameCol($colId,$userId,$new_name) {
		...
		$return['msg'] = "Namens�nderung in ".$new_name." erfolgreich";
		return $return;
	}
	...
	function deletebuilding($fieldId,$colId,$userId) {
		...
		if ($build['bev_pro'] > 0) {
			...
			elseif (($data['bev_used'] > $data['max_bev'] - $build['bev_pro']) && ($field['aktiv'] == 1)) {
				$return['msg'] = "Das Geb�ude konnte nicht demontiert werden da von dem Wohnraum noch Siedler arbeiten";
				return $return;
		}
		}
	...
	function mincost($buildingId,$userId,$colId) {
		...
		for ($i=0;$i<count($result);$i++) {
			...
			if (mysql_num_rows($r_ress) == 0) {
				...
				$return['code'] = 0;
				$return['msg'] = "Zum Bau werden ".$result[$i]['count']." ".$gname['name']." ben�tigt - Vorhanden sind aber nur ".$count."";
				return $return;
			}
	...
	function terraform($fieldId,$colId,$terraform,$userId) {
		...
		if (($terraform == 16) && ($data['type'] != 5) && ($data['type'] != 7)) {
			$return['msg'] = "Die Tiefensprengung kann aufgrund der planetaren Integrit�t nur auf Feld 31 erfolgen.";
			$return['code'] = -1;
			return $return;
		}
		if ($terraform == 15) {
			...
			if ($data['type'] != 14) {
				$return['msg'] = "Es k�nnen nur Felsen gesprengt werden.";
				$return['code'] = -1;
				return $return;
			}
		}
		if ($terraform == 19) {
			$data = $this->getgroundfielddatabyid($fieldId,$colId);
			if ($data['type'] != 15) {
				$return['msg'] = "Die Bohrungen k�nnen nur auf einem freigesprengten Feld durchgef�rt werden.";
				$return['code'] = -1;
				return $return;
			}
		}
		...
		if (($col['energie'] < 10) && ($terraform == 1)) {
			$return['msg'] = "Nicht gen�gend Energie vorhanden.";
			$return['code'] = -1;
			return $return;
		}
		...
		if ($terraform == 4) {
			...
			if ($col['energie'] < 10) {
				$return['msg'] = "Es wird 10 Energie ben�tigt, vorhanden wird nur $return['msg']e];
				return $return;
			}
			...
			if ($col['energie'] < 35) {
				$return['msg'] = "Es wird 35 Energie ben�tigt, vorhanden wird nur $return['msg']e];
				return $return;
			}
			if ($good['bm']['count'] < 50) {
				$return['msg'] = "Es wird 50 Baumaterial ben�tigt, vorhanden ist nur $return['msg']m];
				return $return;
			}
			...
			if (mysql_affected_rows() == 0) mysql_query("DELETE FROM stu_colonies_storage WHERE goods_id=3 AND colonies_id=".$colId."",$this->dblink);
			$return['msg'] = "Leite Terraforming ein....Wasserfl�che zugesch�ttet.";
			$return['code'] = 1;
			return $return;
		} elseif (($terraform == 1) && ($data['type'] == 2)) {
			if ($col['energie'] < 35) {
				$return['msg'] = "Es wird 35 Energie ben�tigt, vorhanden ist aber nur $return['msg']e];
				return $return;
			}
			if ($good['bm']['count'] < 50) {
				$return['msg'] = "Es wird 50 Baumaterial ben�tigt, vorhanden ist nur $return['msg']m];
			...
			mysql_query("UPDATE stu_colonies_storage SET count=count-50 WHERE goods_id='3' AND colonies_id='".$colId."' AND count>50",$this->dblink);
			if (mysql_affected_rows() == 0) mysql_query("DELETE FROM stu_colonies_storage WHERE goods_id=3 AND colonies_id=".$colId."",$this->dblink);
			$return['msg'] = "Leite Terraforming ein...Eisfl�che zu Wiese umgewandelt.";
			$return['code'] = 1;
			return $return;
		} elseif ($terraform == 16) {
			if ($good['torp']['count'] < 25) {
				$return['msg'] = "Es werden 25 Photonentorpedos ben�tigt - Vorhanden sind nur $return['msg']p]['count'];
				return $return;
			}
			if ($col['energie'] < 30) {
				$return['msg'] = "Nicht gen�gend Energie vorhanden";
			...
			if ($good['torp']['count'] < 25) {
				...
				$return['msg'] = "Es werden 25 Photonentorpedos ben�tigt - Vorhanden sind nur ".$count;
				return $return;
			}
			if ($col['energie'] < 30) {
				$return['msg'] = "Nicht gen�gend Energie vorhanden";
			...
			if ($good['torp']['count'] < 15) {
				...
				$return['msg'] = "Es werden 15 Photonentorpedos ben�tigt - Vorhanden sind nur ".$count;
				return $return;
			}
			if ($good['dura']['count'] < 15) {
				if ($good['dura']['count'] == 0) $count = 0;
				else $count = $good['dura']['count'];
				$return['msg'] = "Es wird 15 Duranium ben�tigt - Vorhanden ist nur ".$count;
				return $return;
			}
			if ($col['energie'] < 30) {
				$return['msg'] = "Nicht gen�gend Energie vorhanden";
	...
	function upgradebuilding($fieldId,$colId,$userId) {
		...
		if ($data['buildings_id'] == 3) {
			...
			if ($data['buildings_id'] != 3) {
				$return['msg'] = "Es k�nnen nur H�user zu einer Stadt upgegradet werden";
				$return['code'] = -1;
				return $return;
			}
			$coldata = $this->getcolonybyid($colId);
			...
			$coldata = $this->getcolonybyid($colId);
			if ($coldata['energie'] - $build2['eps_cost'] < 0) {
				$return['msg'] = "Es wird ".$build2['eps_cost']." Energie ben�tigt - Vorhanden ist aber nur $return['msg']e];
				$return['code'] = 0;
				return $return;
			}
		...
		$return['msg'] = "Geb�udeupgrade von $return['msg']e]." auf $return['msg']e]." erfolgreich";
		$return['code'] = 1;
		return $return;
	}
	...
	function activateorbitBuilding($fieldId,$colId,$userId) {
		...
		$return['msg'] = $build['name']." auf Feld ".($field['field_id']+1)." aktiviert";
		$return['code'] = 1;
		} else $return['msg'] = "Es werden ".$build['bev_use']." freie Arbeiter ben�tigt";
		return $return;
	...
	function buildship($colId,$classId,$userId) {
		...
		if ((mysql_num_rows($result) == 0) && ($class['ewwerft'] == 1)) {
			$return['msg'] = "Zum Bau eines Schiffes wird eine erweiterte Weerft ben�tigt";
			$return['code'] = 0;
			return $return;
		}
		$data = $this->getcolonybyid($colId);
		if ($data['energie'] - $class['eps_cost'] < 0) {
			$return['msg'] = "Es wird ".$class['eps_cost']." Energie ben�tigt - Vorhanden ist aber nur $return['msg']e];
			return $return;
		}
	...
	function shipmincost($classId,$userId,$colId) {
		...
		for ($i=0;$i<count($result);$i++) {
			...
			if ($result[$i]['count'] < $r_ress) {
				...
				$return['msg'] = "Es werden ".$result[$i]['count']." ".$goodname." ben�tigt - Vorhanden sind nur ".$r_ress;
				$return['code'] = 0;
				return $return;
			}
		}
	...
	function replikator($source,$end,$count,$colId,$userId) {
		...
		if (($source != 1) && ($source != 3) && ($source != 4) && ($source != "torpho") && ($source != "torpla") && ($source != "torqua") && ($source != 20)) {
			$return['msg'] = "Es k�nnen nur Nahrung, Baumaterial, Biomimetisches Gel und Iridium-Erz zur Replikation genutzt werden";
			return $return;
		}
		if ($end == 7) {
	...
	function repairship ($shipId,$colId,$userId) {
		...
		if (mysql_num_rows($result) == 0) {
			$return['msg'] = "Zur Reperatur wird ein aktivierter Raumbahnhof ben�tigt";
			return $return;
		}
		...
		if (mysql_num_rows($result) == 0) {
			$return['msg'] = "Zur Reperatur wird eine Werft ben�tigt";
			return $return;
		}
		$cost = $this->getrepaircost($shipId,$colId);
		if ($cost[1] > $data['energie']) {
			$return['msg'] = "Zur Reperatur wird ".$cost[1]." Energie ben�tigt - Vorhanden ist nur $return['msg']e];
		}
		$countbm = $this->getcountbygoodid(3,$colId);
		if ($cost[3] > $countbm['count']) {
			$return['msg'] = "Zur Reperatur werden ".$cost[3]." Baumaterial ben�tigt - Vorhanden sind nur $return['msg']m]['count'];
			return $return;
		}
		$countdu = $this->getcountbygoodid(6,$colId);
		if ($cost[6] > $countdu['count']) {
			$return['msg'] = "Zur Reperatur werden ".$cost[6]." Duranium ben�tigt - Vorhanden sind nur $return['msg']u]['count'];
			return $return;
		}
		$counttr = $this->getcountbygoodid(9,$colId);
		if ($cost[9] > $countbm['count']) {
			$return['msg'] = "Zur Reperatur werden ".$cost[9]." Tritanium ben�tigt - Vorhanden sind nur $return['msg']r]['count'];
			return $return;
		}
		$countke = $this->getcountbygoodid(12,$colId);
		if ($cost[12] > $countbm['count']) {
			$return['msg'] = "Zur Reperatur werden ".$cost[12]." Kelbnoit ben�tigt - Vorhanden sind nur $return['msg']e]['count'];
			return $return;
		}
	...
	function demontship($colId,$shipId,$userId) {
		...
		if ($shipdata['user_id'] != $userId) {
			$return['msg'] = "Das Schiff geh�rt Dir nicht";
			return $return;
		}
	...
	function repairbuilding($colId,$mode,$fieldId,$userId) {
		...
		for ($i=0;$i<count($cost);$i++) {
			...
			if ($count['count'] < $rcost) {
				...
				$return['msg'] = "Es wird ".$rcost." ".$cost[$i]['good']['name']." ben�tigt - Vorhanden ist nur ".$scount;
				return $return;
			}
		}
		for ($i=0;$i<count($cost);$i++) {
	...
	function defendcolony($colId,$shipId) {
		...
		for ($i=0;$i<mysql_num_rows($result);$i++) {
			...
			if ($count['count'] > 0) {
				$msg .= "Torpedoplattform  (Feld ".($data['field_id']+1).") schie�t einen ".$type." auf die $data['field_id']e]."<br>";
				if ($shipdata['schilde_aktiv'] == 1) {
	...
	function buildcheck($colId,$userId) {
		...
		$return['msg'] = "Die Koloniezentrale auf dieser Kolonie wurde zers�rt.";
		return $return;
	}
	...
	function addplanet($x,$y,$type,$wese) {
		...
		{
			$return['msg'] = "Der Planet kann nur im freien Raum eingef�gt werden";
			return $return;
		}
		if ($type < 1 || $type > 10)
		{
			$return['msg'] = "Ung�ltiger Typ";
			return $return;
		}
