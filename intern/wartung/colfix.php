...
while ($data = mysql_fetch_assoc($cols)) {
	...
	for ($j = 0; $j < 54; $j++) {
		...
		if ($fields[$j]['aktiv'] == 1) {
			...
			//echo "Oberfl�che-Feld: ".$j." - Geb�ude$build['name']ame]." - bev_use$build['name']use]." - Insgesamt: ".$bev_used."<br>";
		}
	}
	...
	for ($j = 0; $j < 18; $j++) {
		...
		if ($orbit[$j]['aktiv'] == 1) {
			if ($build['bev_use'] > 0) $bev_used = $bev_used + $build['bev_use'];
			if ($build['bev_pro'] > 0) $bev_pro = $bev_pro + $build['bev_pro'];
			//echo "Orbit-Feld: ".$j." - Geb�ude: $build['name']e]." - bev_use: $build['name']e]." - Insgesamt: ".$bev_used."<br>";
		}
	}
	...
	for ($j = 0; $j < 27; $j++) {
		...
		if ($ground[$j]['aktiv'] == 1) {
			if ($build['bev_use'] > 0) $bev_used = $bev_used + $build['bev_use'];
			if ($build['bev_pro'] > 0) $bev_pro = $bev_pro + $build['bev_pro'];
			//echo "Untergrund-Feld: ".$j." - Geb�ude: $build['name']e]." - bev_use: $build['name']e]." - Insgesamt: ".$bev_used."<br>";
		}
	}
