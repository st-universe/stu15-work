	...
	if ($nw == 9)
	{
		...
		if (rand(1,100) < $chg)
		{
			...
			if ($dmgA['integrity']-$dmg > 0)
			{
				...
				$myDB->query("INSERT INTO stu_pms (recipient,sender,message,date,cate) VALUES ('".$data['user_id']."','2','Auf der Kolonie ".stripslashes($data['name'])." wurde das Geb�ude auf Feld "$data['user_id']d]+1)." durch einen Sandsturm um ".$dmg." besch�digt',NOW(),'4')");
			}
		}
	}
	if (($nw == 4) && (mysql_num_rows($res2) == 0))
	{
		...
		if (rand(1,100) < $chg)
		{
			if ($data['bev_free'] > 0)
			{
				...
				$myDB->query("INSERT INTO stu_pms (recipient,sender,message,date,cate) VALUES ('".$data['user_id']."','2','Auf der Kolonie ".stripslashes($data['name'])." sind ".$rand." Einwohner durch �berschwemmungen ums Leben gekommen',NOW(),'4')");
			}
		}
	}
