...
while($data=mysql_fetch_assoc($cols))
{
	...
	{
		$streik = 1;
		$trm .= "<br>Aufgrund des Nahrungsmangels auf der Kolonie ".addslashes(stripslashes($data['name']))." ist die Produktion um 80% eingeschr�nkt.<br>Es sind ".$myDB->query("SELECT bev_free FROM stu_colonies WHERE id=$data['name']d],1)." Einwohner ausgewandert";
