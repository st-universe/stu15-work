...
-- Zuletzt ge�ndert: 23.03.04 (dj)
-- Erstellt am: 14.12.03 (dj)
-- Version-History:
---0.3 (dj)
---{
--- Steuerung f�r das Wurmloch hinzugef�gt (Ausgangs�nderung alle 2 Stunden)
---}
---0.2 (dj)
---{
--- Steuerung f�r einen wandernden Ionensturm hinzugef�gt
---}
---0.1 (dj)
---{
--- Script zur Steuerung von stellaren Ph�nomenen wie Ionenst�rme, Subraumspalten, etc
---}
*/
// Steuerung einzelner Ionenst�rme in der Arcadia-Ausdehnung
...
// Steuerung des gro�en Ionensturms
...
while ($data = mysql_fetch_assoc($result)) {
	$rand = rand(1, 4);
	if ($rand == 1 && $data['coords_x'] + 1 <= $mapfields['max_x']) {
		$newx = $data['coords_x'] + 1;
		$newy = $data['coords_y'];
	}
	if ($rand == 2 && $data['coords_x'] - 1 >= 1) {
		$newx = $data['coords_x'] - 1;
		$newy = $data['coords_y'];
	}
	if ($rand == 3 && $data['coords_y'] + 1 <= $mapfields['max_y']) {
		$newx = $data['coords_x'];
		$newy = $data['coords_y'] + 1;
	}
	if ($rand == 4 && $data['coords_y'] - 1 >= 1) {
		$newx = $data['coords_x'];
		$newy = $data['coords_y'] - 1;
	}
