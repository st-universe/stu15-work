...
while ($trade = mysql_fetch_assoc($traderes)) {
	...
	if ($trade['user_id'] == $userId) $pm .= "Ihr Angebot " . $trade['id'] . " war l�nger als 7 Tage in der Warenb�rse und wurde deshalb gel�scht";
	if ($trade['user_id'] != $userId) {
		...
		$pm = "Ihr Angebot " . $trade['id'] . " war l�nger als 7 Tage in der Warenb�rse und wurde deshalb gel�scht<br>";
		$lid = $trade['id'];
	}
}
if ($lid != $trade['id']) {
