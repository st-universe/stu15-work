...
elseif ($section == "showcolony")
{
	...
	<tr>
		...
		<tr>
			...
			if ($myColony->cweather == 1) { $wt = "bew"; $walt = "Bew�lkung"; }
			...
			if ($myColony->cweather == 5) { $wt = "sauere"; $walt = "S�ureregen"; }
...
elseif ($section == "field")
{
	...
	if ($fieldData['data']['buildtime'] > 0 && time() < $fieldData['data']['buildtime'])
	{
		...
		{
			$badd = "Technikerteam noch nicht wieder verf�gbar.<br>Verf�gbar am ".date("d.m.Y H$fieldData['data']ata]['buildtime'])." <script type=\"text/javascript\">";
		}
		else
		{
			$badd = "Dieses Geb�ude befindet sich zur Zeit in Bau.<br>Fertigstellung am ".date("d.m.Y H:i$fieldData['data']a]['buildtime'])." <script type=\"text/javascript\">";
	...
	if ($fieldData['build']['id'] > 0) echo "Integrit�t: $fieldData['build']a]['integrity']."/$fieldData['build']d]['integrity']."<br><br>";
...
elseif ($section == "orbitfield")
{
	...
	if (($fieldData['data']['buildtime'] > 0) && (time() < $fieldData['data']['buildtime']))
	{
		$badd = "<br>Dieses Geb�ude befindet sich zur Zeit in Bau.<br>Fertigstellung am ".date("d.m.Y H:i$fieldData['data']a]['buildtime'])." <script type=\"text/javascript\">
...
elseif ($section == "beam")
{
	...
	else
	{
		...
		else
		{
			...
			$crew = "<img src=".$grafik."/buttons/crew.gif title='Verf�gbare Crew auf dem Schiff'> $shipdata['crew']w]." | <img src=".$grafik."/bev_unused_1_".$myUser->urasse.".gif title='Freier Wohnraum auf der Kolonie'> ".($myColony->cbev_used+$myColony->cbev_free > $myColony->cmav_bev ? "0" : $myColony->cmav_bev-$myColony->cbev_used-$myColony->cbev_free);
			$beam = "b_from1.gif";
			$nw = "to";
		}
...
elseif ($section == "repairship")
{
	...
	else
	{
		...
		<td class=tdmain align=Center>Schiffsreparatur: ".stripslashes($shipdata['name'])." (H�lle: $shipdata['name']e]."/$shipdata['name']l].")</td></tr>
		<tr>
	...
	for ($i=0;$i<count($modules['huelle']);$i++)
	{
		...
		if (($modules['huelle'][$i]['view'] == 1) || ($stor != 0))
			echo "<tr><td class=tdmainobg width=300><input type=radio name=huellmod value=\"".$modules['huelle'][$i]['id']."\"".$dis."".$sel."> ".$myColony->getgoodpicbyid($modules['huelle'][$i]['goods_id'])." ".$modules['huelle'][$i]['name']."</td>
			 ...
			 <td class=tdmainobg width=200>H�lle: $modules['huelle']e][$i]['huell']."</td></tr>";
	}
