...
if (!$section) {
	...
	<tr>
		<td class=tdmainobg>Bev�lkerung: " . ($myColony->cbev_used + $myColony->cbev_free) . "/" . $myColony->cmax_bev . " (" . $bev_add_show . ")</td>
	</tr>
	<tr>
		<td class=tdmainobg>";
	...
	for ($i = 1; $i <= $myColony->cmax_bev; $i++) {
		if ($ub > 0) {
			if ($ub >= 10 && $gb + 10 <= $myColony->cmax_bev) {
				$beva .= "<img src=" . $bevpath . "/bev_used_5_" . $bevrace . ".gif border=0>";
				$ub -= 10;
				$j += 26;
				$gb += 10;
			} else {
				$beva .= "<img src=" . $bevpath . "/bev_used_1_" . $bevrace . ".gif border=0>";
				$ub -= 1;
				$j += 11;
				$gb += 1;
			}
		...
		if ($fb > 0 && $ub == 0) {
			if ($fb >= 10 && $gb + 10 <= $myColony->cmax_bev) {
				$beva .= "<img src=" . $bevpath . "/bev_unused_5_" . $bevrace . ".gif border=0>";
				$fb -= 10;
				$j += 26;
				$gb += 10;
			} else {
				$beva .= "<img src=" . $bevpath . "/bev_unused_1_" . $bevrace . ".gif border=0>";
				$fb -= 1;
				$j += 11;
				$gb += 1;
			}
		...
		if ($wr > 0 && $ub == 0 && $fb == 0) {
			if ($wr >= 10) {
				$beva .= "<img src=" . $bevpath . "/bev_free_5_" . $bevrace . ".gif border=0>";
				$wr -= 10;
				$j += 26;
				$gb += 10;
			} else {
				$beva .= "<img src=" . $bevpath . "/bev_free_1_" . $bevrace . ".gif border=0>";
				$wr -= 1;
				$j += 11;
				$gb += 1;
			}
	...
	if ($fb > 0 || $ub > 0) {
		...
		for ($i = 1; $i <= $ob; $i++) {
			if ($ob >= 10) {
				$beva .= "<img src=" . $bevpath . "/bev_over_5_" . $bevrace . ".gif border=0>";
				$ob -= 10;
				$j += 26;
			} else {
				$beva .= "<img src=" . $bevpath . "/bev_over_1_" . $bevrace . ".gif border=0>";
				$ob -= 1;
				$j += 11;
			}
			if ($j >= 600) {
	...
	if ($myColony->cweather == 1) {
		$wt = "bew";
		$walt = "Bew�lkung";
	}
	if ($myColony->cweather == 2) {
		$wt = "gewitt";
		$walt = "Gewitter";
	}
	if ($myColony->cweather == 3) {
		$wt = "meteor";
		$walt = "Meteroitenschauer";
	}
	if ($myColony->cweather == 4) {
		$wt = "regen";
		$walt = "Regen";
	}
	if ($myColony->cweather == 5) {
		$wt = "sauere";
		$walt = "S�ureregen";
	}
	if ($myColony->cweather == 6) {
		$wt = "schnee";
		$walt = "Schneefall";
	}
	if ($myColony->cweather == 7) {
		$wt = "sonne";
		$walt = "Sonnenschein";
	}
	if ($myColony->cweather == 8) {
		$wt = "ssand";
		$walt = "Sandsturm";
	}
	if ($myColony->cweather == 9) {
		$wt = "sschnee";
		$walt = "Schneesturm";
	}
	...
	echo "<tr><td class=tdmainobg colspan=2><a href=main.php?page=colony&section=showsectorflights&id=" . $id . " onMouseOver=\"cp('secflight','buttons/secflight2')\" onMouseOut=\"cp('secflight','buttons/secflight1')\"><img src=" . $grafik . "/buttons/secflight1.gif name=secflight border=0> Sektordurchfl�ge</a> (" . $myColony->getflightcount($myColony->cid) . ")</td></tr>";
	...
	echo "<tr><td class=tdmainobg colspan=2><img src=" . $grafik . "/buttons/leavecol1.gif name=leavecol border=0> Kolonie <a href=?page=colony&section=evcol&id=" . $id . " onMouseOver=\"cp('leavecol','buttons/leavecol2')\" onMouseOut=\"cp('leavecol','buttons/leavecol1')\">aufgeben</a> / <a href=?page=colony&section=detcol&id=" . $id . " onMouseOver=\"cp('leavecol','buttons/leavecol3')\" onMouseOut=\"cp('leavecol','buttons/leavecol1')\">sprengen</a></td></tr></table>
			...
			<td class=tdmainobg valign=top width=250><b>Bev�lkerung</b><br>
	...
	echo "<tr><td class=tdmainobg>Name �ndern</td></tr>
		<tr>
			<form action=main.php method=post>
			<input type=hidden name=page value=colony>
			<input type=hidden name=section value=showcolony>
			<input type=hidden name=id value=" . $id . ">
			<td class=tdmainobg><input type=text name=rename class=text size=50 value=\"" . htmlentities(stripslashes($myColony->cname)) . "\"> <input type=submit class=button value=�ndern></td>
		</tr></form>";
	if ($myColony->cmax_schilde > 0) echo "<form action=main.php method=post>
			...
			<tr><td class=tdmainobg><input type=text size=2 maxlength=2 class=text name=freq1 value='" . $myColony->cschild_freq1 . "'>,<input type=text size=1 maxlength=1 class=text name=freq2 value='" . $myColony->cschild_freq2 . "'> <input type=submit value=�ndern class=button></td></tr>
			</form>";
	echo "</table></td>
	</tr>
	</table>";
	...
	if ($fieldData['data']['type'] == 6) $feld = "W�ste";
	...
	if ($fieldData['data']['type'] == 38) $feld = "Ger�st";
	...
	if ($fieldData['build'] == 0) {
		...
		if ((($fieldData['build']['id'] < 210) || ($fieldData['build']['id'] > 215)) && ($fieldData['build']['id'] != 218)) $chgn = "Name: <input class=text type=text size=25 name=buildname value=\"" . $fieldData['data']['name'] . "\"> <input type=submit class=button value=�ndern>";
	...
	if (($fieldData['build']['id'] == 1) || ($fieldData['build']['id'] == 23) || ($fieldData['build']['id'] == 63) || ($fieldData['build']['id'] == 64) || ($fieldData['build']['id'] == 65) || ($fieldData['build']['id'] == 66) || ($fieldData['build']['id'] == 89) || ($fieldData['build']['id'] == 163) || ($fieldData['build']['id'] == 178)) $kolzen = "<br><br>
			...
			<a href=main.php?page=colony&section=ebuild&id=" . $id . " onMouseOver=document.e_build.src='" . $grafik . "/buttons/e_trans2.gif' onMouseOut=document.e_build.src='" . $grafik . "/buttons/e_trans1.gif'><img src=" . $grafik . "/buttons/e_trans1.gif name=e_build border=0> Geb�udeschaltung</a>";
	...
	if ($fieldData['data']['buildings_id'] >= 210 && $fieldData['data']['buildings_id'] <= 215) {
		...
		<table width=350 bgcolor=#262323 cellspacing=1 cellpadding=1>
		<tr><td class=tdmainobg align=Center colspan=2><strong>Botschaft an Allianz �bergeben</strong></td></tr>";
		...
		$botschaft .= "<tr>
			<td colspan=2 align=center class=tdmainobg><input type=submit value=�bergeben class=button name=send ></td>
		</tr>
		</table></form>";
	}
	if ($fieldData['data']['buildtime'] > 0 && time() < $fieldData['data']['buildtime']) {
		if ($fieldData['data']['buildings_id'] == 218) {
			$badd = "Technikerteam noch nicht wieder verf�gbar.<br>Verf�gbar am " . date("d.m.Y H:i", $fieldData['data']['buildtime']) . " <script type=\"text/javascript\">";
		} else {
			$badd = "Dieses Geb�ude befindet sich zur Zeit in Bau.<br>Fertigstellung am " . date("d.m.Y H:i", $fieldData['data']['buildtime']) . " <script type=\"text/javascript\">";
		}
		...
		$sid = 106;
		if ($fieldData['data']['buildings_id'] != 218) {
			$fieldData['data']['aktiv'] == 0 ? $anb = "Ja - <a href=?page=colony&section=showcolony&id=" . $id . "&field=" . $field . "&action=chbam&m=2>�ndern</a>" : $anb = "Nein - <a href=?page=colony&section=showcolony&id=" . $id . "&field=" . $field . "&action=chbam&m=1>�ndern</a>";
	...
	echo "<table width=100% cellpadding=1 cellspacing=1 bgcolor=#262323>
	<tr>
		<td class=tdmain>/ <a href=?page=colony>Kolonien</a> / <a href=main.php?page=colony&section=showcolony&id=" . $id . ">" . $myColony->cname . "</a> / <strong>Oberfl�chenfeld " . ($field + 1) . "</strong>" . $nadd . "</td>
	</tr>
	</table><br>
	...
	if ($fieldData['build']['id'] > 0) echo "Integrit�t: " . $fieldData['data']['integrity'] . "/" . $fieldData['build']['integrity'] . "<br><br>";
	echo $badd . $bdadd . $status . $kolzen . $upgrade . $research . $spy . $shield . $cloakf . $swap . $botschaft . "<br>" . $chgn;
	echo "</td><td valign=top class=tdmainobg width=210><b>Baumen�</b><br>";
	if ($fieldData['build']['id'] == 0) {
		...
		if (mysql_num_rows($result) == 0) echo "Keine Geb�ude f�r dieses Feld vorhanden";
		else {
			while ($data = mysql_fetch_assoc($result)) {
				if ($data['id'] == 210) {
	...
	if ($fieldData['data']['type'] == 37) $feld = "Asteroideng�rtel";
	if ($fieldData['build'] == 0) {
		$status = "nicht bebaut";
		$tadd = $feld;
	} else {
		$chgn = "Name: <input class=text type=text size=25 name=buildname value=\"" . $fieldData['data']['name'] . "\"> <input type=submit class=button value=�ndern>";
		$tadd = "<strong>" . $fieldData['build']['name'] . "</strong>";
	...
	if (($fieldData['data']['buildtime'] > 0) && (time() < $fieldData['data']['buildtime'])) {
		$badd = "<br>Dieses Geb�ude befindet sich zur Zeit in Bau.<br>Fertigstellung am " . date("d.m.Y H:i", $fieldData['data']['buildtime']) . " <script type=\"text/javascript\">
		...
		$sid = 106;
		$fieldData['data']['aktiv'] == 0 ? $anb = "Ja - <a href=?page=colony&section=showcolony&id=" . $id . "&field=" . $field . "&action=chobam&m=2>�ndern</a>" : $anb = "Nein - <a href=?page=colony&section=showcolony&id=" . $id . "&field=" . $field . "&action=chobam&m=1>�ndern</a>";
		$bdadd = "Aktiviert nach Bau: " . $anb . "<br><br>";
	} else $sid = $fieldData['build']['id'];
	if ($fieldData['build']['id'] == 0) {
	...
	echo "</td><td valign=top class=tdmainobg width=200><b>Baumen�</b><br>";
	if ($fieldData['build'] == 0) {
		$user < 100 ? $result = $myDB->query("SELECT a.type,b.id,b.research_id,b.name FROM stu_field_build as a LEFT JOIN stu_buildings as b ON a.buildings_id=b.id WHERE a.type=" . $fieldData['data']['type'] . " AND a.buildings_id!=1 AND a.buildings_id!=23 AND a.buildings_id!=89 AND (a.buildings_id<63 OR a.buildings_id>66) ORDER by b.name") : $result = $myDB->query("SELECT a.buildings_id,a.type,b.id,b.research_id,b.name FROM stu_field_build as a LEFT OUTER JOIN stu_buildings as b ON a.buildings_id=b.id WHERE a.type=" . $fieldData['data']['type'] . " AND a.buildings_id!=1 AND a.buildings_id!=23 AND a.buildings_id!=89 AND (a.buildings_id<63 OR a.buildings_id>66) AND b.view=1 AND b.level<=" . $myUser->ulevel . " ORDER by b.name");
		if (mysql_num_rows($result) == 0) echo "Keine Geb�ude f�r dieses Feld vorhanden";
		else {
	...
	if (($fieldData['data']['buildtime'] > 0) && (time() < $fieldData['data']['buildtime'])) {
		...
		$sid = 106;
		if ($fieldData['data']['aktiv'] == 0) $anb = "Ja - <a href=?page=colony&section=showcolony&id=" . $id . "&field=" . $field . "&action=chubam&m=2>�ndern</a>";
		else $anb = "Nein - <a href=?page=colony&section=showcolony&id=" . $id . "&field=" . $field . "&action=chubam&m=1>�ndern</a>";
		$bdadd = "Aktiviert nach Bau: " . $anb . "<br><br>";
	} else $sid = $fieldData['build']['id'];
	if ($fieldData['build']['id'] == 0) {
		$pic = "<img src=" . $grafik . "/fields/" . $fieldData['data']['type'] . ".gif>";
	...
	<tr>
		<td valign=top class=tdmainobg width=330>" . $tadd . "<br>" . $pic . "<br>";
	if ($fieldData['build']['id'] > 0) echo "Integrit�t: " . $fieldData['data']['integrity'] . "/" . $fieldData['build']['integrity'] . "<br><br>";
	echo $status . $upgrade . $badd . $bdadd . "<br>" . $chgn . "</td><td valign=top class=tdmainobg width=200><b>Baumen�</b><br>";
	if ($fieldData['build'] == 0) {
		$terraform = $myColony->getpossibleterraforming($fieldData['data']['type']);
		$user < 100 ? $result = $myDB->query("SELECT a.type,b.id,b.research_id,b.name FROM stu_field_build as a LEFT JOIN stu_buildings as b ON a.buildings_id=b.id WHERE a.type=" . $fieldData['data']['type'] . " AND a.buildings_id!=1 AND a.buildings_id!=23 AND a.buildings_id!=89 AND (a.buildings_id<63 OR a.buildings_id>66) ORDER by b.name") : $result = $myDB->query("SELECT a.buildings_id,a.type,b.id,b.research_id,b.name FROM stu_field_build as a LEFT OUTER JOIN stu_buildings as b ON a.buildings_id=b.id WHERE a.type=" . $fieldData['data']['type'] . " AND a.buildings_id!=1 AND a.buildings_id!=23 AND a.buildings_id!=89 AND (a.buildings_id<63 OR a.buildings_id>66) AND b.view=1 AND b.level<=" . $myUser->ulevel . " ORDER by b.name");
		if (mysql_num_rows($result) == 0) echo "Keine Geb�ude f�r dieses Feld vorhanden";
	...
	echo "<table width=100% cellpadding=1 cellspacing=1 bgcolor=#262323>
	<tr>
		<td class=tdmain>/ <a href=?page=colony>Kolonien</a> / <a href=main.php?page=colony&section=showcolony&id=" . $id . ">" . $myColony->cname . "</a> / <a href=?page=colony&section=field&field=" . $field . "&id=" . $id . ">Oberfl�chenfeld " . ($field + 1) . "</a> / <strong>Geb�ude demontieren</strong></td>
	</tr>
	</table><br>
	<table width=500 cellpadding=1 cellspacing=1 bgcolor=#262323>";
	if ($data == 0) echo "<tr><td align=center class=tdmainobg width=100%>Kein Geb�ude vorhanden</td></tr>";
	else {
		...
		<tr>
			<td class=tdmainobg>Soll das Geb�ude wirklich demontiert werden?<br>
			<a href=main.php?page=colony&section=showcolony&action=delete&id=" . $id . "&field=" . $field . "><font color=Red>Best�tigung</font></a></td>
		</tr>";
	}
	echo "</table>";
	...
	echo "<table width=100% cellpadding=1 cellspacing=1 bgcolor=#262323>
	<tr>
		<td class=tdmain>/ <a href=?page=colony>Kolonien</a> / <a href=main.php?page=colony&section=showcolony&id=" . $id . ">" . $myColony->cname . "</a> / <a href=?page=colony&section=orbitfield&field=" . $field . "&id=" . $id . ">Orbitfeld " . ($field + 1) . "</a> / <strong>Geb�ude demontieren</strong></td>
	</tr>
	</table><br>
	<table width=500 cellpadding=1 cellspacing=1 bgcolor=#262323>";
	if ($data == 0) echo "<tr><td colspan=2 align=Center class=tdmainobg>Kein Geb�ude vorhanden</td></tr>";
	else {
		$building = $myColony->getbuildbyid($data['buildings_id']);
		if ($building['secretimage'] != "0") {
		...
		<tr>
			<td class=tdmainobg>Soll das Geb�ude wirklich demontiert werden?<br>
			<a href=main.php?page=colony&section=showcolony&action=orbitdelete&id=" . $id . "&field=" . $field . "><font color=Red>Best�tigung</font></a></td>
		</tr>";
	}
	echo "</table>";
	...
	echo "<table width=100% cellpadding=1 cellspacing=1 bgcolor=#262323>
	<tr>
		<td class=tdmain>/ <a href=?page=colony>Kolonien</a> / <a href=main.php?page=colony&section=showcolony&id=" . $id . ">" . $myColony->cname . "</a> / <a href=?page=colony&section=groundfield&field=" . $field . "&id=" . $id . ">Untergrundfeld " . ($field + 1) . "</a> / <strong>Geb�ude demontieren</strong></td>
	</tr>
	</table><br>
	<table width=500 cellpadding=1 cellspacing=1 bgcolor=#262323>";
	if ($data == 0) echo "<tr><td colspan=2 align=Center class=tdmainobg>Kein Geb�ude vorhanden</td></tr>";
		...
		<tr>
			<td class=tdmainobg>Soll das Geb�ude wirklich demontiert werden?<br>
			<a href=main.php?page=colony&section=showcolony&action=grounddelete&id=" . $id . "&field=" . $field . "><font color=Red>Best�tigung</font></a></td>
		</tr>";
	}
	echo "</table>";
} elseif ($section == "beam") {
	...
	if (!$shipid) {
		...
		<tr>
			<td class=tdmain colspan=4>Zielobjekt ausw�hlen</td>
		</tr>";
		...
		if ($way == "to") {
			...
			$crew = "<img src=" . $grafik . "/buttons/crew.gif title='Verf�gbare Crew auf dem Schiff'> " . $shipdata['crew'] . " | <img src=" . $grafik . "/bev_unused_1_" . $myUser->urasse . ".gif title='Freier Wohnraum auf der Kolonie'> " . ($myColony->cbev_used + $myColony->cbev_free > $myColony->cmav_bev ? "0" : $myColony->cmav_bev - $myColony->cbev_used - $myColony->cbev_free);
			$beam = "b_from1.gif";
			$nw = "to";
		}
	...
	if (!$shipid) {
		...
		if (mysql_num_rows($result) == 0) echo "<tr><td class=tdmainobg colspan=4>Es befinden sich keine besch�digten Schiffe im Orbit</td></tr>";
		else {
			while ($data = mysql_fetch_assoc($result)) {
		...
		<td class=tdmain align=Center>Schiffsreparatur: " . stripslashes($shipdata['name']) . " (H�lle: " . $shipdata['huelle'] . "/" . $shipdata['maxhuell'] . ")</td></tr>
		<tr>
	...
	<tr>
		...
		<a href=?page=colony&section=showcolony&action=evcol&id=" . $id . "><font color=red>Best�tigung</font></a></td>
	</tr>
	</table>";
	...
	<tr>
		...
		<a href=?page=colony&section=showcolony&action=detcol&id=" . $id . "><font color=red>Best�tigung</font></a></td>
	</tr>";
	...
	if (!$shipid) {
		...
		<td class=tdmain colspan=3>Zielobjekt ausw�hlen</td></tr>
		...
		echo "<table width=350 cellpadding=1 cellspacing=1 bgcolor=#262323>
			...
			<a href=?page=colony&section=showcolony&action=demontship&id=" . $id . "&shipid=" . $shipid . "><font color=Red>Best�tigung</font></a></td></tr></table>";
	}
} elseif ($section == "ebuild") {
	echo "<table width=100% cellpadding=1 cellspacing=1 bgcolor=#262323>
	<tr>
		<td class=tdmain>/ <a href=?page=colony>Kolonien</a> / <a href=main.php?page=colony&section=showcolony&id=" . $id . ">" . $myColony->cname . "</a> / <strong>Geb�udeschaltung</strong></td>
	</tr>
	</table><br>";
	if ($sent == 1) {
	...
	<tr><td class=tdmain align=center>Geb�ude auf der Oberfl�che</td></tr>";
	...
	echo "</select></td></tr>
		...
		<input type=hidden name=id value=" . $id . ">
		<input type=hidden name=sent value=1>
		<table cellpadding=1 cellspacing=1 bgcolor=#262323><tr><td class=tdmain align=center>Geb�ude im Orbit</td></tr><tr><td class=tdmain>
	...
	if ($myColony->ccolonies_classes_id != 6 && $myColony->ccolonies_classes_id != 9) {
		...
		<input type=hidden name=id value=" . $id . ">
		<input type=hidden name=sent value=1>
		<table cellpadding=1 cellspacing=1 bgcolor=#262323><tr><td class=tdmain align=center>Geb�ude im Untergrund</td></tr><tr><td class=tdmain>
	...
	echo "<table width=100% cellpadding=1 cellspacing=1 bgcolor=#262323>
	<tr>
		<td class=tdmain>/ <a href=?page=colony>Kolonien</a> / <a href=main.php?page=colony&section=showcolony&id=" . $id . ">" . $myColony->cname . "</a> / <strong>Geb�udereperatur</strong></td>
	</tr>
	</table><br>";
	...
	<tr>
		<td class=tdmainobg>Die Koloniezentrale auf diesem Planeten wurde zerst�rt<br>
	...
	echo "<table width=100% cellpadding=1 cellspacing=1 bgcolor=#262323>
	<tr>
		<td class=tdmain>/ <a href=?page=colony>Kolonien</a> / <a href=main.php?page=colony&section=showcolony&id=" . $id . ">" . $myColony->cname . "</a> / <strong>Sektordurchfl�ge</strong></td>
	</tr>
	</table><br>
	<table width=100% cellpadding=1 cellspacing=1 bgcolor=#262323>";
	if (mysql_num_rows($flights) == 0) echo "<tr><td class=tdmainobg align=center colspan=3>Keine Durchfl�ge</td></tr>";
	else {
	...
	if (!$shipid || $shipdata == 0) {
		...
		echo "<table width=100% bgcolor=#262323 cellspacing=1 cellpadding=1>
		<tr>
			<td class=tdmain colspan=5>Zielobjekt ausw�hlen</td>
		</tr>";
		if (mysql_num_rows($result) == 0) echo "<tr><td class=tdmainobg colspan=5>Es befinden sich keine Schiffe im Orbit</td></tr>";
		else {
			echo "<tr><td class=tdmainobg></td><td class=tdmainobg></td><td class=tdmainobg></td><td class=tdmainobg>eingebaut</td><td class=tdmainobg>verf�gbar</td></tr>";
			while ($data = mysql_fetch_assoc($result)) {
				$data['huelldam'] < 40 && $data['trumfield'] == 0 && $data['ships_rumps_id'] != 111 ? $mpf = "d/" : $mpf = "";
				if ($data['secretimage'] != "0") {
					$shippic = "<img src=http://www.stuniverse.de/gfx/secret/" . $mpf . $data['secretimage'] . ".gif border=0>";
				...
				echo "<tr>
					...
					<td class=tdmainobg>" . ($data['tachyon'] == 0 ? "<img src=" . $grafik . "/buttons/decloak.gif title='Tachyon-Emitter Upgrade verf�gbar'>" : "") . "</td>
				</tr>";
			}
		}
		echo "</table>";
		...
		echo "<table width=500 cellpadding=1 cellspacing=1 bgcolor=#262323>
		<tr>
			<td width=45% class=tdmainobg><strong>Verf�gbare Upgrades</strong></td>
			<td width=1%></td>
	...
	<table width=300 cellpadding=1 cellspacing=1 bgcolor=#262323>
	<tr>
		<td class=tdmainobg>Verf�gbare Energie: " . $myColony->cenergie . " - Schildladung: " . $myColony->cschilde . "/" . $myColony->cmax_schilde . "</td>
	</tr>
	<form action=main.php method=post>
	<input type=hidden name=page value=colony>
	<input type=hidden name=id value=" . $id . ">
	...
	<tr>
		<td class=tdmainobg colspan=5><strong>H�lle</strong></td>
	</tr>
	<form action=main.php method=post>
	<input type=hidden name=page value=colony>
	...
	for ($i = 0; $i < count($modules['huelle']); $i++) {
		...
		if (($modules['huelle'][$i]['view'] == 1) || ($stor != 0))
			echo "<tr><td class=tdmainobg width=300><input type=radio name=huellmod value=\"" . $modules['huelle'][$i]['id'] . "\"" . $dis . "" . $sel . "> " . $myColony->getgoodpicbyid($modules['huelle'][$i]['goods_id']) . " " . $modules['huelle'][$i]['name'] . "</td>
			 ...
			 <td class=tdmainobg width=200>H�lle: " . $modules['huelle'][$i]['huell'] . "</td></tr>";
	}
	...
	<tr>
		...
		<td class=tdmain><strong>H�lle</strong></td>
