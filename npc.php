<BODY bgcolor="#000000" text="#FFFFFF">
	...
	<td class=tdmainobg width=*>" . $name . "</td>";
		...
		if ($sys4 > 0) {
			$module = $myShip->getmodulebyid($sys4);
			$mods .= "<img src=" . $grafik . "/goods/" . $module['goods_id'] . ".gif> ";
		}
		echo "<input type=hidden name=antrieb[" . $num . "] value=" . $sys5 . ">";
		if ($sys5 > 0) {
			$module = $myShip->getmodulebyid($sys5);
			$mods .= "<img src=" . $grafik . "/goods/" . $module['goods_id'] . ".gif> ";
		}
	...
	if (!$section || ($section == "main")) {
		...
		<tr>
		<td width=75% class=tdmain align=center><strong>NPC-Men�</strong></td>
	</tr>";
		...
		<tr>
			...
			echo "</select> Koords: <input type=text size=3 name=coords_x class=text> / <input type=text size=3 name=coords_y class=text> - Sektor: <input type=text size=1 name=wese class=text> <input type=submit value=Ausr�sten class=button></td><td class=tdmainobg align=right><a href=main.php?page=npc&section=shipclassesdb>Schiffsklassen Datenbank</a></td>
		</tr><tr><td class=tdmainobg></td><td class=tdmainobg align=right><a href=main.php?page=npc&section=buildingsdb&user=" . $user . "&pass=" . $pass . ">Geb�ude Datenbank</a></td></tr></form></table>";
		}
		if ($myUser->uhalfnpc == 1) {
			echo "<tr>
	...
	<tr>
		...
		if ($user == 19) {
			...
			$name  = 'Kessok Torpedogesch�tz';	// Name
			writeblueprintoption(3);
		}
		if ($user == 26) {
			$sys1  = 3;		// H�lle
			...
			$name  = 'Yridianischer Zerst�rer';	// Name
			writeblueprintoption(5);
			...
			$name  = 'Nausicaanischer Zerst�rer';	// Name
			writeblueprintoption(6);
		...
		<tr>
			<td class=tdmain><strong>Schiff ausr�sten</strong></td>
		</tr>";
			$data = $myShip->getmodulesbytype(1, $user);
			if ($data != 0) {
				echo "<tr><td class=tdmain>H�llenmodul</td></tr>";
		...
		echo "<table width=100% bgcolor=#262323 cellspacing=1 cellpadding=1>
	<tr>
		<td width=100% class=tdmain align=center><strong>NPC-Men�</strong></td>
	</tr>
	<tr>
	...
	<tr>
		<td width=100% class=tdmain align=center><b>Geb�ude�bersicht</b></td>
	</tr>
	</table>
	<br>
	<table width=400>";
	...
	<tr>
		<td width=100% class=tdmain align=center><b>NPC-Geb�ude / nicht baubar</b></td>
	</tr>
	</table>
	<br>
	<table width=400>";
