...
class colony
{
	...
	function buildonfield($fieldId,$buildingId,$colId)
	{
		...
		{
			$return['msg'] = "Das Geb�ude ($return['msg']e].") kann erst ab Level $return['msg']l]." gebaut werden.";
			$return['code'] = 0;
			return $return;
		}
		if ($building['research_id'] > 0)
		{
			...
			{
				$return['msg'] = "Dieses Geb�ude wurde noch nicht erforscht";
				$return['code'] = 0;
				return $return;
			}
		}
		...
		{
			$return['msg'] = "Zum Bau werden ".$building['eps_cost']." Energie ben�tigt - Vorhanden ist aber nur ".$this->cenergie;
			$return['code'] = 0;
			return $return;
		}
		if (($building['id'] == 7) || ($building['id'] == 17) || ($building['id'] == 33) || ($building['id'] == 34) || ($building['id'] == 74) || ($building['id'] == 75) || ($building['id'] == 76))
		{
			...
			{
				$class["mine".$building['id']] == 0 ? $return['msg'] = $building['name']." ist auf diesem Planeten nicht baubar" : $return['msg'] = "Es k�nnen keine weiteren Geb�ude von diesem Typ$building['id']ame].") errichtet werden";
				return $return;
			}
		}
		...
		if ($building['id'] == 82)
		{
			...
			if ($count == 3)
			{
				$return['msg'] = "Es k�nnen nur maximal 3 Schildbatterien gebaut werden";
				return $return;
			}
		}
		if ($building['id'] == 83)
		{
			...
			if ($count >= 4)
			{
				$return['msg'] = "Es k�nnen maximal 4 Minengef�ngnisse gebaut werden";
				return $return;
			}
		}
		if ($building['id'] == 86)
		{
			...
			{
				$return['msg'] = "Es k�nnen maximal 3 $return['msg']e] ."gebaut werden";
				return $return;
			}
		}
		...
		$return['msg'] = "Der Bau des Geb�udes ($return['msg']e].") auf Feld "$return['msg']d]+1)." wird am ".date("d.m.Y H:i",(time($return['msg']e]))." beendet sein";
		$return['code'] = 1;
		return $return;
	}
	...
	function deactivateBuilding($fieldId,$colId)
	{
		...
		{
			$return['msg'] = "Das Geb�ude konnte nicht deaktiviert werden, da sonst einige Arbeiter obdachlos w�ren";
			return $return;
		}
	...
	function activateBuilding($fieldId,$colId,$userId)
	{
		...
		{
			$return['msg'] = "Das Geb�ude besitzt keine Aktivierungsfunktion";
			return $return;
		}
		...
		if ($field['aktiv'] == 1)
		{
			$return['msg'] = "Das Geb�ude ist bereits aktiviert";
			return $return;
		}
		if ($field['buildtime'] > 0)
		{
			$return['msg'] = "Dieses Geb�ude kann nicht aktiviert werden, da es sich noch in Bau befindet";
			return $return;
		}
		if ($data['bev_free'] < $build['bev_use'] && $build['bev_pro'] == 0)
		{
			$return['msg'] = "Zum Aktivieren werden ".$build['bev_use']." Arbeiter ben�tigt - Es sind jedoch nur $return['msg']e]." Kolonisten frei";
			$return['code'] = 0;
			return $return;
		}
		global $myUser;
		...
		{
			$return['msg'] = "Zum aktivieren des Geb�udes wird Level $return['msg']l]." ben�tigt";
			return $return;
		}
		if ($build['research_id'] > 0 && $this->getuserresearch($build['research_id'],$userId) == 0)
		{
			$return['msg'] = "Du kannst dieses Geb�ude nicht aktivieren, da Du es noch nicht erforscht hast";
			return $return;
		}
	...
	function renameCol($new_name)
	{
		...
		$return['msg'] = "�nderung des Namens in ".stripslashes($new_name)." erfolgreich";
		return $return;
	}
	...
	function deletebuilding($fieldId)
	{
		...
		{
			$return['msg'] = "Dieses Geb�ude kann nicht demontiert werden";
			return $return;
		}
		global $myAlly;
	...
	function mincost($buildingId)
	{
		...
		for ($i=0;$i<count($result);$i++)
		{
			...
			{
				$return['code'] = 0;
				$return['msg'] = "Zum Bau werden ".$result[$i]['count']." ".$result[$i]['name']." ben�tigt - Vorhanden sind aber nur ".$count;
				return $return;
			}
		}
	...
	function terraform($fieldId,$colId,$terraform,$mode)
	{
		...
		{
			$return['msg'] = "Zum sprengen wird ein aktivierter Untergrundlift ben�tigt";
			return $return;
		}
		if ($fieldId == 31 && $tf['save31'] == 1)
		{
			$return['msg'] = "Dieses Feld kann aufgrund der planetaren Integrit�t nicht gesprengt werden";
			return $return;
		}
		if ($tf['id'] == 14 && $this->ccolonies_classes_id !=5)
		{
			$return['msg'] = "Tiefenbohrungen k�nnen nuf auf Klasse K Planeten durchgef�hrt werden";
			return $return;
		}
		...
		{
			$return['msg'] = "Dieses Terraforming ist nur einmal m�glich";
			return $return;
		}
		if ($tf['flimit'] > 0)
		{
			...
			{
				$return['msg'] = "Dieses Terraforming ist nur ".$tf['flimit']." mal m�glich";
				return $return;
			}
		}
		if ($data['buildings_id'] > 0)
		{
			$return['msg'] = "Terraforming nicht m�glich - Das Feld ist bebaut";
			return $return;
		}
		if ($tf['ecost'] > $this->cenergie)
		{
			$return['msg'] = "F�r das Terraforming werden $return['msg']t]." Energie ben�tigt - Vorhanden ist nur ".$this->cenergie;
			return $return;
		}
	...
	function tfmincost($terraformId)
	{
		...
		while($tf=mysql_fetch_assoc($result))
		{
			...
			{
				$return['code'] = 0;
				$return['msg'] = "Zum Terraformen werden ".$tf['count']." ".$tf['name']." ben�tigt - Vorhanden sind aber nur ".$count;
				return $return;
			}
		}
		$result = $this->gettfcost($terraformId);
	...
	function beamto($id,$goodId,$count)
	{
		...
		if ($this->db->query("SELECT id FROM stu_torpedo_types WHERE goods_id=".$goodId,1) != 0)
		{
			...
			{
				$return['msg'] = "Es k�nnen maximal $return['msg']c]['torps']." Torpedos geladen werden";
				return $return;
			}
	...
	function upgradebuilding($fieldId,$colId)
	{
		...
		if ($data['buildtime'] != 0)
		{
			$return['msg'] = "Das Geb�ude wurde noch nicht fertiggestellt";
			return $return;
		}
		global $myUser;
		if ($data['buildings_id'] == 3)
		{
			if ($data['aktiv'] == 0)
			{
				$return['msg'] = "Die H�user auf diesem Feld sind nicht aktiviert";
				return $return;
			}
			...
			{
				$return['msg'] = "St�dte k�nnen erst ab Leve$return['msg']vel]." errichtet werden";
				$return['code'] = -1;
				return $return;
			}
			if ($this->cenergie - $build2['eps_cost'] < 0)
			{
				$return['msg'] = "Es wird ".$build2['eps_cost']." Energie ben�tigt - Vorhanden ist aber nur ".$this->cenergie;
				$return['code'] = 0;
				return $return;
			}
			...
			$return['msg'] = "Der Bau des Geb�udes ($return['msg']e].") auf Feld ".($fieldId+1)." wird am ".date("d.m.Y H:i",(time($return['msg']e]))." beendet sein";
			return $return;
		}
		elseif ($data['buildings_id'] == 21)
		{
			...
			if ($this->cenergie - $build2['eps_cost'] < 0)
			{
				$return['msg'] = "Es wird ".$build2['eps_cost']." Energie ben�tigt - Vorhanden ist aber nur ".$this->cenergie." Energie";
				return $return;
			}
			...
			{
				$return['msg'] = "Es k�nnen maximal 2 interstellare Raumh�fen gebaut werden";
				return $return;
			}
		...
		elseif ($data['buildings_id'] == 1)
		{
			...
			{
				$return['msg'] = "Dieses Geb�ude muss erst erforscht werden";
				return $return;
			}
			if ($this->ccolonies_classes_id != 1)
			{
				$return['msg'] = "Dieses Geb�ude ist auf diesem Planeten nicht baubar";
				return $return;
			}
			if ($this->cenergie - $build2['eps_cost'] < 0)
			{
	...
	function groundupgrade($fieldId,$colId)
	{
		...
		if ($data['buildtime'] != 0)
		{
			$return['msg'] = "Geb�ude noch nicht fertiggestellt!";
			return $return;
		}
		if ($data['buildings_id'] == 36)
		{
			...
			if ($this->cenergie - $build2['eps_cost'] < 0)
			{
				$return['msg'] = "Es ist nicht gen�gend Energie vorhanden";
				$return['code'] = 0;
				return $return;
			}
			$result = $this->mincost($building,$this->user,$colId);
	...
	function orbitbuild($fieldId,$buildingId,$colId)
	{
		...
		{
			$return['msg'] = "Zum Orbitbau wird ein aktivierter Raumbahnhof ben�tigt";
			return $return;
		}
		...
		{
			$return['msg'] = "Es k�nnen maximal 3 Materie-Extraktoren gebaut werden";
			return $return;
		}
		...
		{
			$return['msg'] = "Es k�nnen maximal 2 Genesis-Forschungsstationen gebaut werden";
			return $return;
		}
		...
		{
			$return['msg'] = "Es k�nnen maximal 4 Verarbeitungsstationen gebaut werden";
			return $return;
		}
	...
	function activateorbitBuilding($fieldId,$colId,$userId)
	{
		...
		if ($this->db->query("SELECT id FROM stu_colonies_fields WHERE (buildings_id=21 OR buildings_id=168 OR buildings_id=192) AND colonies_id=".$colId." AND aktiv=1",1) == 0)
		{
			$return['msg'] = "Zum Aktivieren wird ein aktivierter Raumbahnhof ben�tigt";
			return $return;
		}
		if ($field['buildtime'] > 0)
		{
		...
		{
			$return['msg'] = "Es werden ".$build['bev_use']." freie Arbeiter ben�tigt";
			return $return;
		}
		global $myUser;
		if ($myUser->getfield("level",$userId) < $build['level'])
	...
	function groundbuild($fieldId,$buildingId)
	{
		...
		{
			$return['msg'] = "Zum Bau im Untergrund wird ein aktivierter Untergrundlift ben�tigt";
			return $return;
		}
		...
		if (($building['id'] == 7) || ($building['id'] == 17) || ($building['id'] == 33) || ($building['id'] == 34))
		{
			...
			if ($count >= $class["mine".$building['id']])
			{
				...
				else $return['msg'] = "Es k�nnen keine weiteren Geb�ude von diesem Typ$return['msg']ame].") errichtet werden";
				return $return;
			}
		}
		if ($building['id'] == 82)
	...
	function activategroundBuilding($fieldId,$colId,$userId)
	{
		...
		{
			$return['msg'] = "Zum Aktivieren wird ein aktivierter Untergrundlift ben�tigt";
			return $return;
		}
		...
		if ($data['bev_free'] < $build['bev_use'])
		{
			$return['msg'] = "Zum Aktivieren des Geb�udes werden $return['msg']e]." Arbeiter ben�tigt";
			return $return;
		}
		global $myUser;
		if ($myUser->getfield("level",$userId) < $build['level'])
	...
	function deletegroundbuilding($fieldId)
	{
		...
		{
			$return['msg'] = "Zum demontieren wird ein aktivierter Untergrundlift ben�tigt";
			return $return;
		}
		$build = $this->getbuildbyid($field['buildings_id']);
		if ($field['aktiv'] == 1)
	...
	function buildship($huellmod,$schildmod,$waffenmod,$sensormod,$antriebmod,$reaktormod,$epsmod,$computermod,$classId)
	{
		...
		{
			$return['msg'] = "Zum Bau eines Schiffes wird ein aktivierter Raumbahnhof ben�tigt";
			return $return;
		}
		global $myShip;
		...
		if ($class['slots'] > 0)
		{
			$return['msg'] = "Stationen k�nnen nicht in Werften gebaut werden";
			return $return;
		}
		...
		{
			$return['msg'] = "Zum Bau dieses Schiffes wird eine Werft ben�tigt";
			return $return;
		}
		...
		{
			$return['msg'] = "Zum Bau dieses Schiffes wird eine erweiterte Werft ben�tigt";
			return $return;
		}
		...
		{
			$return['msg'] = "Es sind ".round($wirtsum,2)." Punkte vorhanden - Zum Bau werden ".round(($points+$wirtdata),2)." ben�tigt";
			return $return;
		}
		if ($this->cenergie - $class['eps_cost'] < 0)
		{
			$return['msg'] = "Es wird ".$class['eps_cost']." Energie ben�tigt - Vorhanden ist aber nur ".$this->cenergie;
			return $return;
		}
	...
	function shipmincost($classId,$userId,$colId)
	{
		...
		for ($i=0;$i<count($result);$i++)
		{
			...
			{
				$return['msg'] = "Es werden ".$result[$i]['count']." ".$result[$i]['name']." ben�tigt - Vorhanden sind nur ".$r_ress;
				$return['code'] = 0;
				return $return;
			}
		}
	...
	function research($researchId)
	{
		...
		{
			$return['msg'] = "Nicht gen�gend Iso-Chips vorhanden";
			return $return;
		}
	...
	function loadbatt($id,$count)
	{
		...
		if ($this->db->query("SELECT id FROM stu_colonies_fields WHERE (buildings_id=21 OR buildings_id=168) AND colonies_id=".$this->cid." AND aktiv=1",1) == 0)
		{
			$return['msg'] = "Zum Aufladen der Ersatzbatterie wird ein aktivierter Raumbahnhof ben�tigt";
			return $return;
		}
		if (($this->ccoords_x != $shipdata['coords_x']) || ($this->ccoords_y != $shipdata['coords_y']))
		{
			$return['msg'] = "Schiff und Kolonie m�ssen sich im selben Sektor befinden.";
			return $return;
		}
		...
		{
			$return['msg'] = "Die Reservebatterien von Stationen k�nnen nicht in einer Werft aufgeladen werden";
			return $return;
		}
		if ($count == "max") $count = $this->cenergie;
		if ($this->cenergie < $count) $count = $this->cenergie;
	...
	function fabrik($end,$count,$buildID)
	{
		...
		{
			$return['msg'] = "Es befindet sich keine fertiggestellte geeignete Fabrik f�r dieses Modul auf der Kolonie";
			return $return;
		}
		if ($ferror == 1)
		{
		...
		for ($i=0;$i<count($modcost);$i++)
		{
			...
			{
				$return['msg'] = "Es werden ".($count*$modcost[$i]['count'])." ".$modcost[$i]['name']." ben�tigt - Vorhanden sind nur ".$storcount;
				return $return;
			}
		}
	...
	function raffinerie($source,$count)
	{
		...
		{
			$return['msg'] = "Es k�nnen nur Kelbonit-Erz und Nitrum-Erz veredelt werden";
			return $return;
		}
	...
	function repairship ($shipId)
	{
		...
		if ($shipdata['c']['slots'] != 0)
		{
			$return['msg'] = "Stationen k�nnen nicht in einer Werft repariert werden";
			return $return;
		}
		...
		{
			$return['msg'] = "Zur Reparatur wird ein aktivierter Raumbahnhof ben�tigt";
			return $return;
		}
		...
		{
			$return['msg'] = "Zur Reparatur wird eine Werft ben�tigt";
			return $return;
		}
	...
	function ewopt($mode)
	{
		...
		$return['msg'] = "Einwanderungseinstellung ge�ndert";
		return $return;
	}
	...
	function teleskop($coordsx,$coordsy)
	{
		...
		if ($this->db->query("SELECT id FROM stu_colonies_fields WHERE (buildings_id=21 or buildings_id=168) AND colonies_id=".$this->cid." AND aktiv=1",1) == 0)
		{
			$return['msg'] = "Zum scannen wird ein aktivierter Raumbahnhof ben�tigt";
			return $return;
		}
		if ($this->cenergie < 3)
		{
			$return['msg'] = "Es werden mindestens 3 Energie ben�tigt - Vorhanden sind nur ".$this->cenergie;
			return $return;
		}
		...
		{
			$return['msg'] = "Dieser Sektor kann aufgrund von Subraumst�rungen nicht gescant werden";
			return $return;
		}
	...
	function repairbuilding($mode,$fieldId)
	{
		...
		{
			$return['msg'] = "Dieses Geb�ude ist nicht besch�digt";
			return $return;
		}
		...
		for ($i=0;$i<count($cost);$i++) {
			...
			if ($count < $rcost)
			{
				$return['msg'] = "Es wird ".$rcost." ".$cost[$i]['name']." ben�tigt - Vorhanden ist nur ".$count;
				return $return;
			}
		}
	...
	function coldefensefire($colId,$shipId,$damage)
	{
		...
		if ($shipdata['schilde_aktiv'] == 1)
		{
			if ($shipdata['schilde'] - $damage <= 0)
			{
				...
				else $msg .= "H�llenbruch - Das Schiff wurde zerst�rt";
			}
			else
			{
		...
		else
		{
			...
			if ($huell <= 0) $msg .= "H�llenbruch - Das Schiff wurde zerst�rt";
			else
			{
		...
		if ($huell < 1)
		{
			...
			$coldata = $this->getcolonybyid($colId);
			$myHistory->addEvent("Die ".addslashes($shipdata['name'])." wurde in Sektor ".$shipdata['coords_x']."/".$shipdata['coords_y'].($shipdata['wese'] == 2 ? " (2)" : "")." von der Orbitalverteidigung der Kolonie ".addslashes($coldata['name'])." zerst�rt$shipdata['name']d],1);
			$return['destroyed'] = 1;
		}
		$return['msg'] = $msg;
		return $return; 
	...
	function defendcolony($colId,$shipId)
	{
		...
		for ($i=0;$i<mysql_num_rows($result);$i++)
		{
			...
			$msg .= "Phaserbatterie  (Feld ".($data['field_id']+1).") schie�t auf die $data['field_id']e]."<br>";
		...
		for ($i=0;$i<mysql_num_rows($result);$i++)
		{
			...
			$data = mysql_fetch_assoc($result);
			$msg .= "Disruptorphalanx  (Feld ".($data['field_id']+1).") schie�t auf die $data['field_id']e]."<br>";
		...
		for ($i=0;$i<mysql_num_rows($result);$i++)
		{
			...
			if ($count['count'] > 0)
			{
				...
				for ($q=0;$q<$shots;$q++)
				{
					$msg .= "Raketenwerfer (Feld ".($data['field_id']+1).") schie�t eine Rakete auf die $data['field_id']e]."<br>";
		...
		for ($i=0;$i<mysql_num_rows($result);$i++)
		{
			...
			if ($count['count'] > 0)
			{
				...
				for ($q=0;$q<$shots;$q++)
				{
					$msg .= "Orbitale Verteidigungsplattform (Feld ".($data['field_id']+1).") schie�t einen Plasmatorpedo auf die $data['field_id']e]."<br>";
		...
		for ($i=0;$i<mysql_num_rows($result);$i++)
		{
			...
			if ($count['count'] > 0)
			{
				$msg .= "Torpedoplattform (Orbitfeld ".($data['field_id']+1).") schie�t einen ".$type." auf die $data['field_id']e]."<br>";
	...
	function set_ewstop($count)
	{
		...
		if ($count == "reset")
		{
			...
			$return['msg'] = "Die Einwanderungsgrenze wurde zur�ckgesetzt";
		}
		else
		{
	...
	function shipupgrade($shipId,$upgradeId)
	{
		...
		if ($this->db->query("SELECT id FROM stu_colonies_fields WHERE (buildings_id=21 OR buildings_id=168) AND colonies_id=".$this->cid." AND aktiv=1",1) == 0)
		{
			$return['msg'] = "F�r die Schiffsupgrades wird ein aktivierter Raumbahnhof ben�tigt";
			return $return;
		}
		if ($this->user != $shipdata['user_id'])
		{
		...
		if ($upgradeId == 1)
		{
			...
			{
				$return['msg'] = "F�r die Schiffsupgrades wird eine erweiterte Werft ben�tigt";
				return $return;
			}
			if ($this->cenergie < 10)
			{
				$return['msg'] = "Es wird 10 Energie ben�tigt - Vorhanden ist nur ".$this->cenergie;
				return $return;
			}
			$good['bm'] = $this->getstoragebygoodid(3,$this->cid);
			if ($good['bm'] == 0 || $good['bm'] < 10)
			{
				$return['msg'] = "Es werden 10 Baumaterial ben�tigt - Vorhanden sind nur $return['msg']m];
				return $return;
			}
			$good['dura'] = $this->getstoragebygoodid(6,$this->cid);
			if ($good['dura'] == 0 || $good['dura'] < 10)
			{
				$return['msg'] = "Es werden 10 Duranium ben�tigt - Vorhanden sind nur $return['msg']a];
				return $return;
			}
			$good['iso'] = $this->getstoragebygoodid(10,$this->cid);
			if ($good['iso'] == 0 || $good['iso'] < 5)
			{
				$return['msg'] = "Es werden 5 Iso Chips ben�tigt - Vorhanden sind nur $return['msg']o];
				return $return;
			}
			$good['gel'] = $this->getstoragebygoodid(19,$this->cid);
			if ($good['gel'] == 0 || $good['gel'] < 2)
			{
				$return['msg'] = "Es werden 2 Gel Packs ben�tigt - Vorhanden sind nur $return['msg']l];
				return $return;
			}
	...
	function loadshields($count)
	{
		...
		{
			$return['msg'] = "Die Schilde k�nnen nicht aufgeladen werden wenn sie aktiviert sind";
			return $return;
		}
		if ($this->cschilde == $this->cmax_schilde)
		{
			$return['msg'] = "Die planetaren Schilde sind bereits vollst�ndig aufgeladen";
			return $return;
		}
		if ($count == "max") $count = $this->cenergie;
		if ($this->cenergie < $count) $count = $this->cenergie;
	...
	function setshieldfreq($freq1,$freq2)
	{
		...
		{
			$return['msg'] = "Es wurde eine ung�ltige Schildfrequenz eingegeben";
			return $return;
		}
		...
		$return['msg'] = "Die neue Schildfrequenz betr�gt ".$freq1.",".$freq2."";
		return $return;
	}
	...
	function activateshields()
	{
		...
		if ($this->cenergie < 15)
		{
			$return['msg'] = "Zum aktivieren der Schilde werden 15 Energie ben�tigt";
			return $return;
		}
		if ($this->cschilde == 0)
		{
	...
	function modulateshields($colId)
	{
		...
		global $myComm;
		$myComm->sendpm($data['user_id'],2,"Auf der Kolonie ".$data['name']." wurde die Schildfrequenz moduliert. Die neue Frequenz betr�gt nun ".$freq1.",".$freq2,1);
	}
	...
	function decloakgeb($colId)
	{
		...
		if ($this->cenergie < 12)
		{
			$return['msg'] = "Es wird mindestens 12 Energie zum Scan ben�tigt";
			return $return;
		}
		global $myComm;
		$j=0;
	...
	function horchposten($coordsx,$coordsy,$wese=1)
	{
		...
		if ($this->cenergie < 6)
		{
			$return['msg'] = "Zum Scannen wird 6 Energie ben�tigt";
			return $return;
		}
		if (abs($this->ccoords_x - $coordsx) + abs($this->ccoords_y - $coordsy) >= 40)
		{
			$return['msg'] = "Die Scanreichweite des Horchpostens betr�gt 40 Sektoren";
			return $return;
		}
	...
	function finishProcesses()
	{
		...
		while($data=mysql_fetch_assoc($result))
		{
			...
			{
				$myComm->sendpm($data['user_id'],2,"Der Auftrag an Bord des abgest�rzten Jem hadar Schiffes wurde abgeschlossen",$data['user_id']e]);
			}
			else
			{
				$myComm->sendpm($data['user_id'],2,"Auf der Kolonie ".addslashes(stripslashes($data['name']))." wurde ein Geb�ude auf der Oberfl�che fertiggestellt$data['user_id']ame].")$data['user_id']ime]);
			}
		}
		...
		while($data=mysql_fetch_assoc($result))
		{
			...
			$myComm->sendpm($data['user_id'],2,"Auf der Kolonie ".addslashes(stripslashes($data['name']))." wurde ein Geb�ude im Orbit fertiggestellt ($data['user_id']e].")",$data['user_id']e]);
		}
		...
		while($data=mysql_fetch_assoc($result))
		{
			...
			$myComm->sendpm($data['user_id'],2,"Auf der Kolonie ".addslashes(stripslashes($data['name']))." wurde ein Geb�ude im Untergrund fertiggestellt ($data['user_id']e].")",$data['user_id']e]);
		}
	...
	function checkModules($huellmod,$schildmod,$waffenmod,$epsmod,$computermod,$antriebmod,$sensormod,$reaktormod,$classId,$colId,$userId)
	{
		...
		if ($stor == 0)
		{
			$return['msg'] = "Dieses H�llenmodul befindet sich nicht auf dem Planeten";
			$return['code'] = 0;
			return $return;
		}
		if ($schildmod == 0)
		...
		if ($antriebmod == 0)
		{
			$return['msg'] = "Es wurde kein Antriebsmodul ausgew�hlt";
			$return['code'] = 0;
			return $return;
		}
		$module = $this->getmodulebyid($antriebmod);
		...
		{
			$return['msg'] = "Dieses Antriebsmodul kann in den gew�hlten Schiffsrumpf nicht eingebaut werden";
			$return['code'] = 0;
			return $return;
		}
		$stor = $this->checkstoragebygoodid(1,$module['goods_id'],$colId);
	...
	function chbam($fieldId,$mode)
	{
		...
		$return['msg'] = "Aktivierungseinstellung f�r das Geb�ude auf Feld ".($fieldId+1)." ge�ndert";
		return $return;
	}
	...
	function chobam($fieldId,$mode)
	{
		...
		$return['msg'] = "Aktivierungseinstellung f�r das Geb�ude auf Orbitfeld ".($fieldId+1)." ge�ndert";
		return $return;
	}
	...
	function chubam($fieldId,$mode)
	{
		...
		$return['msg'] = "Aktivierungseinstellung f�r das Geb�ude auf Untergrundfeld ".($fieldId+1)." ge�ndert";
		return $return;
	}
	...
	function makelatinum($colId,$count)
	{
		...
		if ($this->cenergie < 10) $return['msg'] = "Zum Pressen werden mindestens 10 Energie ben�tigt - Vorhanden sind jedoch nur ".$this->cenergie;
		$stor = $this->getgoodstorbyid($colId,34);
		if ($stor < 60) $return['msg'] = "Zum Pressen werden mindestens 60 Ring-Materie ben�tigt. Vorhanden sind jedoch nur ".$stor;
		$storbm = $this->getgoodstorbyid($colId,3);
		if ($storbm < 1) $return['msg'] = "Zum Pressen wird mindestens 1 Baumaterial ben�tigt. Vorhanden sind jedoch nur ".$storbm;
	...
	function robotik($sonId,$count)
	{
		...
		if ($this->cenergie < $ecost)
		{
			$return['msg'] = "Es wird ".$ecost." Energie bent�tigt - Vorhanden sind nur ".$this->cenergie;
			return $return;
		}
		...
		for ($i=0;$i<count($probecost);$i++)
		{
			...
			{
				$return['msg'] = "Es werden ".$probecost[$i]['name']." ben�tigt - Vorhanden sind nur ".$storcount;
				return $return;
			}
		}
	...
	function analyse($goodId)
	{
		...
		if ($goodId == 38)
		{
			...
			{
				$return['msg'] = "Osmium-Analyse bereits durchgef�hrt";
				return $return;
			}
			if ($storcount < 5)
			{
				$return['msg'] = "Zur Analyse werden 5 Osmium ben�tigt";
				return $return;
			}
		...
		if ($goodId == 39)
		{
			...
			{
				$return['msg'] = "Kirstall-Analyse bereits durchgef�hrt";
				return $return;
			}
			if ($storcount < 5)
			{
				$return['msg'] = "Zur Analyse werden 5 tholianische Kristalle ben�tigt";
				return $return;
			}
			$this->lowerstoragebygoodid(5,$goodId,$this->cid);
		...
		if (($goodId >= 102) && ($goodId <= 104))
		{
			...
			if ($storcount < 1)
			{
				$return['msg'] = "Zur Analyse wird mindestens eine Plasmakanone ben�tigt";
				return $return;
			}
		...
		if (($goodId >= 142) && ($goodId <= 144))
		{
			...
			if ($storcount < 1)
			{
				$return['msg'] = "Zur Analyse wird mindestens eine Partikelkanone ben�tigt";
				return $return;
			}
			$this->lowerstoragebygoodid(1,$goodId,$this->cid);
		...
		if (($goodId == 97) || ($goodId == 98) || ($goodId == 234))
		{
			...
			if ($storcount < 1)
			{
				$return['msg'] = "Zur Analyse wird mindestens ein Polaronstrahler ben�tigt";
				return $return;
			}
			$this->lowerstoragebygoodid(1,$goodId,$this->cid);
		...
		if ($goodId == 27)
		{
			...
			if ($storcount < 5)
			{
				$return['msg'] = "Zur Analyse werden mindestens f�nf Polarontropedos ben�tigt";
				return $return;
			}
			$this->lowerstoragebygoodid(5,$goodId,$this->cid);
		...
		if ($goodId == 29)
		{
			...
			if ($storcount < 5)
			{
				$return['msg'] = "Zur Analyse werden mindestens f�nf Positrontropedos ben�tigt";
				return $return;
			}
			$this->lowerstoragebygoodid(5,$goodId,$this->cid);
		...
		if ($goodId == 49)
		{
			...
			$return['msg'] = "Verschl�sselung konnte nicht �berwunden werden. Der Computer identifiziert eine Kessok-Signatur.";
			return $return;
		}
		$return['msg'] = "Materialien/Daten k�nnen nicht verarbeitet werden";
		return $return;
	}
	...
	function changeembassyowner($colId,$field,$embassy)
	{
		...
		if ($embassy != 0)
		{
			...
			{
				$return['msg'] = "Dieser Allianz wurde bereits eine Botschaft �bergeben";
				return $return;
			}
			$emb = $this->db->query("SELECT * FROM stu_allys_embassys WHERE colonies_id=".$colId." AND field_id = ".$field,2);
			if ($emb != 0)
			{
				$return['msg'] = "Dieses Botschaftsgeb�ude wird bereits verwendet";
				return $return;
			}
			$this->db->query("UPDATE stu_allys_embassys SET colonies_id=".$colId.", field_id = ".$field." WHERE id=".$embassy);
			$myComm->sendpm($recipient,$this->user,"Die Allianz ".$ally2['name']." hat Ihnen eine Botschaft �berstellt");
			$return['msg'] = "Botschaft wurde �bergeben";
		}
		else
		{
	...
	function jemhadarfunction($fieldId,$type)
	{
		...
		if ($type == 1)
		{
			...
			if ($data['bev_free'] < 5)
			{
				$return['msg'] = "Zum Hochfahren des Reaktors werden 5 freie Arbeiter ben�tigt";
				return $return;
			}
			if ($field['aktiv'] == 1)
			{
				$return['msg'] = "Der Reaktor l�uft bereits";
				return $return;
			}
		...
		if ($type == 2)
		{
			...
			if ($data['bev_free'] < 5)
			{
				$return['msg'] = "Zum Herunterfahren des Reaktors werden 5 freie Arbeiter ben�tigt";
				return $return;
			}
			if ($field['aktiv'] == 0)
			{
				$return['msg'] = "Der Reaktor l�uft noch nicht";
				return $return;
			}
			$this->deactivateBuilding($fieldId,$this->cid,$this->user);
			$this->db->query("UPDATE stu_colonies_fields SET buildtime='".(time()+10800)."' WHERE field_id='".$fieldId."' AND colonies_id='".$this->cid."'");
			$return['msg'] = "Reaktor wurde au�er Betrieb genommen";
			return $return;
		}
		elseif ($type == 3)
		{
			...
			$return['msg'] = "Alle �brigen Waffensysteme sind unbrauchbar, eine Bergung w�re sinnlos";
			return $return;
		}
		elseif ($type == 4)
		{
			...
			$return['msg'] = "Der Computer ist endg�ltig zerst�rt. Weitere Versuche, die Daten zu bergen, sind unm�glich.";
			return $return;
