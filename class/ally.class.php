...
class ally
{
	...
	function checkally()
	{
		return $this->db->query("SELECT allys_id FROM stu_user WHERE id=" . $this->user, 1);
	}

	function getallylist()
	{
		return $this->db->query("SELECT a.id,a.name,b.user as prae FROM stu_allys as a LEFT OUTER JOIN stu_user as b ON a.user_id=b.id ORDER BY a.id");
	}

	function getallyulist()
	{
		return $this->db->query("SELECT id,name FROM stu_allys ORDER BY id");
	}

	function getallybyid($allyId)
	{
		return $this->db->query("SELECT * FROM stu_allys WHERE id='" . $allyId . "'", 4);
	}
	...
	{
		return $this->db->query("SELECT id,user,rasse,lastloginround,UNIX_TIMESTAMP(lastaction) as last_tsp,vac FROM stu_user WHERE allys_id='" . $allyId . "' AND aktiv=1");
	}
	...
	function newally($name, $pass, $descr, $hp)
	{
		...
		$return['msg'] = "Die Allianz wurde gegr�ndet";
		return $return;
	}
	...
	function leaveally($allyId)
	{
		...
		if ($ally['user_id'] == $this->user) {
			if ($ally['vize'] == 0) {
				...
				$return['msg'] = "Die Allianz wurde gel�scht";
				return $return;
	...
	function updateally($allyId, $field, $value, $userId)
	{
		...
		if ($field == "name") {
			...
			$return['msg'] = "Der Allianzname wurde ge�ndert";
		}
		if ($field == "pass") {
			if (strlen($value) < 5) {
			...
			$return['msg'] = "Das Beitrittspasswort wurde ge�ndert";
		}
		if ($field == "descr") {
			$return['msg'] = "Die Allianzbeschreibung wurde ge�ndert";
			$value = addslashes($value);
		}
		if ($field == "hp") {
			$return['msg'] = "Die Homepage wurde ge�ndert";
			$value = addslashes($value);
		}
	...
	function setwork($allyId, $type, $value)
	{
		...
		if ($this->checkfounder($this->user) == 0) {
			$return['msg'] = "Du bist nicht Pr�sident dieser Allianz";
			return $return;
		}
		if ($data['user_id'] == $value) {
			$return['msg'] = "Du bist der Pr�sident der Allianz und kannst keinen weiteren Posten �bernehmen";
			return $return;
		}
		global $myUser;
		if ($type == "user_id") $job = "Pr�sident";
		if ($type == "vize") $job = "Vize-Pr�sident";
		if ($type == "diplo") $job = "Au�enminister";
	...
	function unsetwork($allyId, $type)
	{
		...
		if ($type == "vize") {
			...
			$return['msg'] = "Der Au�enminister wurde entlassen";
		}
		return $return;
	}

	function getbez($allyId)
	{
		return $this->db->query("SELECT *,UNIX_TIMESTAMP(date) as date_tsp FROM stu_allys_beziehungen WHERE allys_id1=" . $allyId . " OR allys_id2=" . $allyId, 2);
	}

	function getubez($allyId)
	{
		return $this->db->query("SELECT type,allys_id1,allys_id2 FROM stu_allys_beziehungen WHERE allys_id1=" . $allyId . " OR allys_id2=" . $allyId);
	}
	...
	function addbez($allyId, $allyId2, $type)
	{
		...
		if ($type == 1) {
			...
			$return['msg'] = "Der Allianz " . $data['name'] . " wurde der Krieg erkl�rt";
			$myComm->sendpm($recipient, $this->user, "Deiner Allianz wurde von der Allianz " . $data2['name'] . " der Krieg erkl�rt");
			$myHistory->addEvent("Die Allianz " . addslashes($data2['name']) . " hat der Allianz " . addslashes($data['name']) . " den Krieg erkl�rt", $userId, 3);
		} elseif ($type == 2) {
			...
			$return['msg'] = "Der Allianz " . $data['name'] . " wurde ein B�ndnis angeboten";
			$myComm->sendpm($recipient, $this->user, "Deiner Allianz wurde von der Allianz " . $data2['name'] . " ein B�ndnis angeboten");
		} elseif ($type == 6) {
			...
			if (($check != 3) && ($check != 4)) {
				$return['msg'] = "F�r dieses Angebot wird Freundschaft oder ein B�ndnis ben�tigt";
				return $return;
			}
	...
	function getbezbyid($allyId)
	{
		return $this->db->query("SELECT * FROM stu_allys_beziehungen WHERE allys_id2=" . $allyId, 4);
	}

	function getbezbetweenbyids($allyId, $allyId2)
	{
		return $this->db->query("SELECT type FROM stu_allys_beziehungen WHERE (allys_id1=" . $allyId . " OR allys_id2=" . $allyId . ") AND (allys_id1=" . $allyId2 . " OR allys_id2=" . $allyId2 . ")", 1);
	}
	...
	function editbez($allyId, $allyId2, $type)
	{
		...
		if ($type == 5) {
			global $myComm;
			if ($bez == 1) $msg = "Die Allianz " . $data2['name'] . " hat die Kriegserkl�rung zur�ckgenommen";
			if ($bez == 2) $msg = "Die Allianz " . $data2['name'] . " hat den Handelsvertrag gek�ndigt";
			if ($bez == 3) $msg = "Die Allianz " . $data2['name'] . " hat die Freundschaft gek�ndigt";
			if ($bez == 4) $msg = "Die Allianz " . $data2['name'] . " hat das B�ndnis gek�ndigt";
			$myComm->sendpm($recipient, $this->user, $msg);
			$this->db->query("DELETE FROM stu_allys_beziehungen WHERE (allys_id1=" . $allyId . " AND allys_id2=" . $allyId2 . ") OR (allys_id1=" . $allyId2 . " AND allys_id2=" . $allyId . ")");
			$return['msg'] = "Die Beziehung zu der Allianz " . $data['name'] . " wurde gel�scht";
			return $return;
		}
	...
	function takebez($allyId, $bezId)
	{
		...
		if ($angebot['type'] == 4) $msg = "Die Allianz " . $data2['name'] . " hat mit der Allianz " . $data['name'] . " ein B�ndnis geschlossen";
	...
	function getangebotbyid($bezId)
	{
		return $this->db->query("SELECT * FROM stu_allys_bez_angebot WHERE id=" . $bezId, 4);
	}

	function getangebote($allyId)
	{
		return $this->db->query("SELECT * FROM stu_allys_bez_angebot WHERE allys_id2=" . $allyId, 2);
	}

	function getsangebote($allyId)
	{
		return $this->db->query("SELECT * FROM stu_allys_bez_angebot WHERE allys_id1=" . $allyId, 2);
	}

	function checkbez($allyId, $allyId2)
	{
		return $this->db->query("SELECT type FROM stu_allys_beziehungen WHERE (allys_id1=" . $allyId . " AND allys_id2=" . $allyId2 . ") OR (allys_id1=" . $allyId2 . " AND allys_id2=" . $allyId . ")", 1);
	}
	...
	function getfield($value, $allyId)
	{
		return $this->db->query("SELECT " . $value . " FROM stu_allys WHERE id=" . $allyId, 1);
	}

	function getbuiltembassys($allyId)
	{
		return $this->db->query("SELECT a.*,b.name as allyname, c.name as colname FROM stu_allys_embassys as a left outer join stu_allys as b on a.allys_id2 = b.id left outer join stu_colonies as c on a.colonies_id = c.id WHERE a.allys_id1=" . $allyId . " AND a.colonies_id != 0 AND a.allys_id2 = b.id");
	}

	function getownedembassys($allyId)
	{
		return $this->db->query("SELECT a.*,b.name as allyname, c.name as colname FROM stu_allys_embassys as a left outer join stu_allys as b on a.allys_id1 = b.id left outer join stu_colonies as c on a.colonies_id = c.id WHERE a.allys_id2=" . $allyId . " AND a.colonies_id != 0 AND a.allys_id1 = b.id");
	}

	function getunbuiltembassys($allyId)
	{
		return $this->db->query("SELECT a.*,b.name as allyname, c.name as colname FROM stu_allys_embassys as a left outer join stu_allys as b on a.allys_id1 = b.id left outer join stu_colonies as c on a.colonies_id = c.id WHERE a.allys_id2=" . $allyId . " AND a.allys_id1 = b.id");
	}

	function getembassybyallys($allyId, $allyId2)
	{
		return $this->db->query("SELECT * FROM stu_allys_embassys WHERE allys_id1=" . $allyId . " AND allys_id2=" . $allyId2, 2);
	}

	function getembassyownerbycolony($colId, $fieldId)
	{
		return $this->db->query("SELECT allys_id2 FROM stu_allys_embassys WHERE colonies_id=" . $colId . " AND field_id=" . $fieldId, 1);
	}

	function getembassybuilderbycolony($colId, $fieldId)
	{
		return $this->db->query("SELECT allys_id1 FROM stu_allys_embassys WHERE colonies_id=" . $colId . " AND field_id=" . $fieldId, 1);
	}

	function getembassyoptions($allyId)
	{
		return $this->db->query("SELECT a.*,b.name FROM stu_allys_embassys as a left outer join stu_allys as b on a.allys_id2 = b.id WHERE a.allys_id1=" . $allyId . " AND a.colonies_id = 0");
	}

	function getembassybez($allyId)
	{
		return $this->db->query("SELECT * FROM stu_allys_beziehungen WHERE (allys_id1=" . $allyId . " OR allys_id2=" . $allyId . ") AND (type=3 OR type=4)", 2);
	}

	function checkembassybuild()
	{
		return $this->db->query("SELECT a.id FROM `stu_allys` as a left outer join stu_user as b  left outer join stu_allys_embassys as c on b.allys_id = a.id on a.id = c.allys_id1 WHERE ( a.user_id = " . $this->user . " OR a.vize = " . $this->user . " OR a.diplo = " . $this->user . " ) AND c.colonies_id = 0 GROUP by a.id", 1);
	}

	function getembassybyid($embassy)
	{
		return $this->db->query("SELECT * FROM stu_allys_embassys WHERE id=" . $embassy);
	}
	...
	function changeembassystyle($embassy, $look)
	{
		...
		if ((($look < 210) || ($look > 213)) && ($look != 215)) {
			$return['msg'] = "Fehler bei der Parameter�bergabe";
			return $return;
		}
		...
		$return['msg'] = "Baustil wurde ge�ndert";
		return $return;
	}
}
