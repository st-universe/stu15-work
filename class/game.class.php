...
class game
{
	...
	{
		return $this->db->query("SELECT *,UNIX_TIMESTAMP(start) as start_tsp from stu_game_rounds WHERE runde=" . $this->db->query("SELECT max(runde) from stu_game_rounds", 1), 4);
	}
	...
	function getvalue($fielddescr)
	{
		return $this->db->query("SELECT value FROM stu_game WHERE fielddescr='" . $fielddescr . "'", 1);
	}

	function getrounds()
	{
		return $this->db->query("SELECT COUNT(id) FROM stu_game_rounds", 1);
	}

	function getTickStats()
	{
		return $this->db->query("SELECT *,UNIX_TIMESTAMP(start) as start_tsp FROM stu_game_rounds ORDER BY runde DESC LIMIT 20", 2);
	}
