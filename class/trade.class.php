...
class trade
{
	...
	function deloffer($offerId,$userId)
	{
		if ($this->db->query("DELETE FROM stu_trade_offers WHERE id='".$offerId."' AND user_id=".$userId,6) > 0)
		{
			...
			$this->db->query("INSERT INTO stu_trade_logs (user_id,aktion,date) VALUES ('".$userId."','Angebot ".$offerId." gel�scht',NOW())");
			$return['msg'] = "Angebot ".$offerId." gel�scht";
			return $return;
		}
	}
	...
	function takeoffer($offerId)
	{
		...
		for ($i=0;$i<count($data);$i++)
		{
			...
			{
				$return['msg'] = "Zum durchf�hren dieser Transaktion werden ".$data[$i]['count']." ".$this->db->query("SELECT name FROM stu_goods WHERE id=".$data[$i]['goods_id'],1)." ben�tigt";
				return $return;
			}
		}
	...
	function takeofferaction($userId)
	{
		...
		{
				$return['msg'] = "Zum durchf�hren dieser Transaktion werden 25 Latinum ben�tigt";
				return $return;
		}
		$good = rand(1,190);
	...
	function buylatinum($goodId,$shipId)
	{
		...
		if ($stor < $price)
		{
			$return['msg'] = "Zum Kauf werden ".$price." ".$name." ben�tigt - Vorhanden sind nur ".$stor;
			return $return;
		}
		...
		$return['msg'] = "Es wurde 1 Latinum f�r ".$price." ".$name." gekauft";
		return $return;
	}
	...
	function selllatinum($goodId,$shipId)
	{
		...
		if ($count < $price)
		{
			$return['msg'] = "Diese Ware ist nicht verf�gbar";
			return $return;
		}
		...
		{
			$return['msg'] = "Es ist nicht gen�gend Lagerraum auf dem Schiff vorhanden";
			return $return;
		}
		...
		$return['msg'] = "Es wurden ".$price." ".$this->db->query("SELECT name FROM stu_goods WHERE id=".$goodId,1)." f�r ein 1 Latinum gekauft";
		return $return;
	}
	...
	function buytech($goodId,$shipId)
	{
		...
		{
			$return['msg'] = "Diese Daten sind derzeit nicht verf�gbar";
			return $return;
		}
		...
		if ($stor < $price['count'])
		{
			$return['msg'] = "Zum Kauf werden ".$price['count']." ".$pname." ben�tigt - Vorhanden sind nur ".$stor;
			return $return;
		}
		...
		$return['msg'] = "Es wurden ".$name." f�r $return['msg']t]." ".$pname." gekauft";
	...
	function informant($postenId,$shipId,$infoId,$informantId,$shipId2)
	{
		...
		if ($infor == 0)
		{
			$return['msg'] = "Der ausgew�hlte Informant ist nicht verf�gbar";
			return $return;
		}
		...
		if ($infoId == 2)
		{
			...
			else $msg = "Der ".$infor['type']." hat geh�rt, dass sich die Kolonie in Sektor $infor['type']x]."/$infor['type']y]." befindet";
			$myComm->sendpm($this->user,3,$msg);
		}
		if ($infoId == 3)
		{
		...
		$return['msg'] = "Die Information wird Dir in K�rze per PM zugesandt";
		return $return;
	}
