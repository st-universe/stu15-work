...
class map
{
	...
	function getrow($x1, $x2, $y, $wese = 1)
	{
		return $this->db->query("SELECT a.coords_x,a.coords_y,a.type,a.race,b.sperrung FROM stu_map_fields as a LEFT JOIN stu_colonies as b USING(coords_x,coords_y,wese) WHERE a.wese=" . $wese . " AND a.coords_x>=" . $x1 . " AND a.coords_x<=" . $x2 . " AND a.coords_y=" . $y, 2);
	}

	function getfieldinfo($x, $y, $wese = 1)
	{
		return $this->db->query("SELECT id FROM stu_ships WHERE coords_x=" . $x . " AND coords_y=" . $y . " AND cloak=0 AND wese=" . $wese . " AND ships_rumps_id!=154 AND ships_rumps_id!=158", 2);
	}

	function getshipcount($x, $y, $wese = 1)
	{
		return $this->db->query("SELECT COUNT(id) FROM stu_ships WHERE coords_x=" . $x . " AND coords_y=" . $y . " AND cloak=0 AND wese=" . $wese . " AND ships_rumps_id!=154 AND ships_rumps_id!=158", 1);
	}

	function getcloakedfieldinfo($x, $y, $wese = 1)
	{
		return $this->db->query("SELECT COUNT(id) FROM stu_ships WHERE coords_x=" . $x . " AND coords_y=" . $y . " AND wese=" . $wese . " AND cloak=1", 1);
	}
	...
	function getfieldbycoords($x, $y, $wese = 1)
	{
		return $this->db->query("SELECT type,race FROM stu_map_fields WHERE wese=" . $wese . " AND coords_x=" . $x . " AND coords_y=" . $y, 4);
	}

	function checksenjammer($x, $y, $wese = 1)
	{
		return $this->db->query("SELECT COUNT(id) FROM stu_ships WHERE ships_rumps_id=166 AND coords_x=" . $x . " AND coords_y=" . $y, 1);
	}

	function editfield($x, $y, $type, $wese = 1)
	{
		$this->db->query("UPDATE stu_map_fields SET type=" . $type . " WHERE coords_x=" . $x . " AND coords_y=" . $y . " AND type!=6 AND type!=7 AND type!=8 AND type!=9 AND type!=10");
	}

	function getfreem()
	{
		return $this->db->query("SELECT a.id,a.coords_x,a.coords_y,a.wese FROM stu_colonies as a LEFT JOIN stu_map_fields as b USING(coords_x,coords_y,wese) WHERE a.colonies_classes_id=1 AND a.user_id=2 AND b.race=0", 2);
	}

	function checkwormhole($x, $y, $wese = 1)
	{
		return $this->db->query("SELECT start_x,start_y,stable FROM stu_wormholes WHERE start_wese=" . $wese . " AND start_x=" . $x . " AND start_y=" . $y, 4);
	}

	function getUserCheck($x1, $x2, $y1, $y2, $userId, $wese = 1)
	{
		return $this->db->query("SELECT a.id FROM stu_map_sectors as a LEFT OUTER JOIN stu_map_sectors_user as b ON a.id=b.map_sectors_id WHERE a.coords_x1 BETWEEN " . $x1 . " AND " . $x2 . " AND a.coords_y1 BETWEEN " . $y1 . " AND " . $y2 . " b.user_id=" . $userId, 3);
	}

	function checkIoSector($x, $y, $wese = 1)
	{
		return $this->db->query("SELECT id FROM stu_map_special WHERE coords_x=" . $x . " AND coords_y=" . $y . " AND wese=" . $wese . " AND type=1", 3);
	}

	function getMapSectorSpecialType($x, $y, $wese = 1)
	{
		return $this->db->query("SELECT type FROM stu_map_special WHERE coords_x=" . $x . " AND coords_y=" . $y . " AND wese=" . $wese, 4);
	}

	function getmapsectors($wese = 1)
	{
		return $this->db->query("SELECT * FROM stu_map_sectors ORDER BY coords_y1,coords_x1", 2);
	}

	function checksektor($sektorId, $wese = 1)
	{
		return $this->db->query("SELECT COUNT(id) FROM stu_map_sectors_user WHERE map_sectors_id=" . $sektorId . " AND user_id=" . $this->user, 1);
	}

	function checkSectorUser($userId, $sectorId)
	{
		return $this->db->query("SELECT COUNT(user_id) FROM stu_map_sectors_user WHERE map_sectors_id=" . $sectorId . " ANd user_id=" . $userId, 1);
	}
