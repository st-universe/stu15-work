...
class usersess
{
	...
	function login($user, $pass, $alog)
	{
		...
		if ($this->db->query("SELECT COUNT(id) FROM stu_user WHERE UNIX_TIMESTAMP(lastaction)>" . (time() - 300) . " AND id>100", 1) >= 55 && $data['id'] > 100) {
			$return['code'] = 0;
			$return['msg'] = "Es sind zur Zeit �ber 55 Spieler eingeloggt. Login nicht m�glich. Bitte versuche es zu einem sp�teren Zeitpunkt nochmal!";
			return $return;
		}
		if ($data['aktiv'] == 2) {
			$return['msg'] = "Dein Account wurde wegen eines Regelversto�es deaktiviert.<br>
