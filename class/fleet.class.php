...
class fleet
{
	...
	function newfleet($shipId, $name)
	{
		...
		if ($data['c']['slots'] > 0) {
			$return['msg'] = "Es kann keine Station als Flaggschiff ausgew�hlt werden";
			return $return;
		}
	...
	{
		return $this->db->query("SELECT COUNT(id) FROM stu_fleets WHERE user_id=" . $this->user, 1);
	}

	function getfleetshipsinfo($fleetId, $shipId)
	{
		return $this->db->query("SELECT a.id,a.ships_rumps_id,a.name,a.energie,a.huelle,a.schilde,a.schilde_aktiv,a.batt,b.name as rname FROM stu_ships as a LEFT JOIN stu_ships_rumps as b ON a.ships_rumps_id=b.id WHERE a.fleets_id=" . $fleetId . " AND a.id!=" . $shipId);
	}

	function getfleetbyshipid($shipId)
	{
		return $this->db->query("SELECT * FROM stu_fleets WHERE ships_id='" . $shipId . "'", 4);
	}

	function getfleetsbyuserid($userId)
	{
		return $this->db->query("SELECT * FROM stu_fleets WHERE user_id='" . $userId . "'", 2);
	}

	function getfleetbyid($fleetId)
	{
		return $this->db->query("SELECT * FROM stu_fleets WHERE id='" . $fleetId . "'", 4);
	}
	...
	function delfleet($shipId)
	{
		...
		$return['msg'] = "Die Flotte wurde aufgel�st";
		return $return;
	}

	function getfleetshipcount($fleetId)
	{
		return $this->db->query("SELECT COUNT(id) FROM stu_ships WHERE fleets_id='" . $fleetId . "'", 1);
	}
	...
	function joinfleet($shipId, $fleetId)
	{
		...
		if ($shipcount == 20) {
			$return['msg'] = "Es k�nnen maximal 20 Schiffe in einer Flotte sein";
			return $return;
		}
	...
	function getshipsbyfleetid($fleetId)
	{
		return $this->db->query("SELECT * FROM stu_ships WHERE fleets_id='" . $fleetId . "'", 2);
	}
	...
	function move($shipId, $x, $y)
	{
		...
		while ($ship = mysql_fetch_assoc($res)) {
			if ($ship['energie'] == 0) {
				...
				$msg .= "Die " . stripslashes($ship['name']) . " hat keine Energie f�r den Flug und l�st sich von der Flotte<br>";
				continue;
			}
	...
	function autopilot($shipId, $way, $fields)
	{
		...
		while ($i <= $fields) {
			if ($this->db->query("SELECT energie FROM stu_ships WHERE id=" . $data['id'], 1) == 0) {
				$msg .= "Das Flaggschiff " . stripslashes($ship['name']) . " hat keine Energie f�r den Flug. Die Flotte stoppt in diesem Sektor<br>";
				break;
			}
	...
	function wormhole($shipId)
	{
		...
		if ($data['c']['crew_min'] - 1 > $data['crew']) {
			$return['msg'] = "Es werden " . ($data['c']['crew_min'] - 1) . " Crewmitglieder auf dem Flaggschiff ben�tigt um in das Wurmloch zu fliegen";
			return $return;
		}
	...
	function chgflag($shipId, $shipId2)
	{
		...
		if ($data2['crew'] < 2) {
			$return['msg'] = "Um die " . $data2['name'] . " zum Flaggschiff zu ernennen m�ssen mindestens 2 Crewmitglieder an Board sein";
			return $return;
		}
