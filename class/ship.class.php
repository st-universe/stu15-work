...
class ship
{
	...
	function move($shipId,$x,$y,$userId,$fleetmove="",$red=1)
	{
		...
		if ($userId != 17 && $data['c']['probe'] == 0 && $this->traked == 0)
		{
			...
			{
				$return['msg'] = "Zum fliegen werden mindestens ".($data['c']['crew_min'] - 2)." Crewmitglieder ben�tigt";
				if ($fleetmove != 0)
				{
					$this->db->query("UPDATE stu_ships SET fleets_id=0 WHERE id=".$data['id']);
					$return['msg'] .= "<br>Die ".stripslashes($data['name'])." hat sich auf der Flotte gel�st";
				}
				return $return;
			}
			if ($data['crew'] < 2)
			{
				$return['msg'] = "Zum fliegen werden mindestens 2 Crewmitglieder ben�tigt";
				return $return;
			}
		}
		if ($data['dock'] > 0)
		...
		if ($data['race'] != $cd['race'] && $cd['race'] != 0)
		{
			...
			if ($cd['race'] == 10) $raceenter .= "<td class=tdmainobg>Schiff fliegt in den Raum der F�deration ein</td></tr>";
		...
		if ($cd['race'] == 15 && $data['cloak'] == 1)
		{
			if (($this->db->query("SELECT count(id) FROM stu_ships WHERE coords_x BETWEEN ($x-1) AND ($x+1) AND coords_y BETWEEN ($y-1) AND ($y+1) AND ships_rumps_id=189",1) != 0) AND ($this->db->query("SELECT id FROM stu_ships_action WHERE ships_id = ".$data['id']." AND mode='borderdet' AND ships_id2 = 15",1) == 0))
			{
				...
				$raceenter .= "<tr><td class=tdmainobg></td><td class=tdmainobg><img src=".$this->gfx."/ships/189.gif></td><td class=tdmainobg>Das Schiff wurde aufgesp�rt</td></tr>";
				if ($userId > 100) $myComm->sendpm($cd['race'],2,"Ann�herungsalarm: Die $cd['race']e]." von ".$myUser->uuser." ($cd['race']d].") wurde bei ".$x."/".$y." entdeckt",2);
			}
		}
		...
		if ($data['traktor'] != 0 && $data['traktormode'] == 1)
		{
			...
			{
				$trw = 0;
				$trak_msg = " - Nicht gen�gend Energie, Traktorstrahl abgerissen";
        ...
        if ($data['traktor'] != 0 && $data['traktormode'] == 2)
		{
			...
			{
				$trak_msg = "Kann nicht von Traktorstrahl der ".$data2['name']." l�sen";
				...
				if ($data2['user_id'] != $userId) $myComm->sendpm($data2['user_id'],$data['user_id'],"Die ".$data['name']." hat versucht sich vom Traktorstrahl der ".$data2['name']." zu l�sen",2);
				$return['msg'] = "Die ".$data['name']." kann sich nicht vom Traktorstrahl der ".$data2['name']." l�sen";
				return $return;
			}
			elseif ($data2['energie'] == 0 && $data2['user_id'] != $data['user_id'])
			{
				if ($data['user_id'] != $data2['user_id']) $myComm->sendpm($data2['user_id'],$data['user_id'],"Die ".$data['name']." hat sich vom Traktorstrahl der ".$data2['name']." gel�st",2);
				$trak_msg = "Vom Traktorstrahl der ".$data2['name']." gel�st";
		...
		if ($cd['type'] == 4 || $cd['type'] == 17 || $cd['type'] == 19)
		{
			...
			if ($huelle < 1) $myHistory->addEvent("Die ".addslashes($data['name'])." wurde durch einen Asteroiden zerst�rt",$userId);
		}
		...
		if ($cd['type'] == 13)
		{
			...
			$energie += 1;
			if ($this->fmsg == 0) $msg .= "<br>Feldtyp: R�ntgenpulsar";
			$sa = 0;
			$cl = 0;
			$ks = 0;
			$ls = 0;
		...
		if ($cd['type'] == 31 && $data['huellmodlvl'] == 54)
		{
			...
			if ($huelle < $mh)
			{
				...
				$huelle += $rand;
				if ($rand != 0) $msg .= "<br>Regeneration: H�lle um ".$rand." Punkte regeneriert";
			}
			$red = 0;
		}
		if ($cd['type'] == 15)
		...
		if ($huelle < $data['huelle'] && $huelle > 0) $msg .= "<br>H�llenschaden: "$data['huelle']e]-$huelle)." - H�lle bei ".$huelle;
		if ($huelle < 1)
		{
			$this->trumfield($shipId);
			$destroy = 1;
			$red = 0;
			$msg .= "<br>Die ".stripslashes($data['name'])." wurde beim Einflug in den Sektor zerst�rt";
			$this->cshow == 0;
		}
		if ($crewkill > 0)
		{
			...
			if ($data['c']['crew_min'] > $crew)
			{
				$cl = 0;
				$sa = 0;
				$msg .= "<br>Systemausf�lle durch fehlende Besatzung";
			}
		...
		if ($data['fleets_id'] > 0 && $fleet == 0 && $fleetmove == 0)
		{
			...
			$fleetmsg = "Die ".$data['name']." hat sich aus der Flotte gel�st<br>";
		}
	...
	function activatevalue($shipId,$value,$userId)
	{
		...
		if ($value == "cloak")
		{
			...
			{
				$return['msg'] = "Die Tarnung kann aufgrund eines stellaren Ph�nomens nicht aktiviert werden";
				return $return;
			}
			...
			{
				$return['msg'] = "Das Schiff ist zu sehr besch�digt um die Tarnung zu aktivieren";
				return $return;
			}
		...
		if ($value == "actscan")
		{
			...
			if ($data['energie'] < 5)
			{
				$return['msg'] = "Um den Active-Scan Modus zu aktivieren werden 5 Energie ben�tigt";
				return $return;
			}
		}
		if ($value == "lss")
		...
		if ($value == "kss")
		{
			...
			if ($myUser->ulevel < 2)
			{
				$return['msg'] = "Diese Funktion steht erst ab Level 2 zur Verf�gung";
				return $return;
			}
			if ($data['crew_min'] - 2 > $data['crew'] && $data['ships_rumps_id'] != 88)
			{
		...
		{
			$return['msg'] = "Aufgrund des Mutaranebels k�nnen die Sensoren nicht aktiviert werden";
			return $return;
		}
		if (($value == "kss" || $value == "lss") && $data['mtype'] == 15 && $data['sensormodlvl'] != 52)
		{
			$return['msg'] = "Aufgrund des Quasars k�nnen die Sensoren nicht aktiviert werden";
			return $return;
		}
		if ($value == "schilde_aktiv")
		{
			...
			if ($data['schilde'] < 1)
			{
				$return['msg'] = "Zum aktivieren der Schilde m�ssen die Schildemitter aufgeladen werden";
				return $return;
			}
			if ($data['dock'] > 0)
			{
			...
			if ($data['mtype'] == 13)
			{
				$return['msg'] = "Die Schilde k�nnen aufgrund des R�ntgenpulsars nicht aktiviert werden";
				return $return;
			}
			if ($data['mtype'] == 21)
			{
				$return['msg'] = "Die Schilde k�nnen aufgrund des ceruleanischen Nebels nicht aktiviert werden";
				return $return;
			}
			if ($data['mtype'] == 2 || $data['mtype'] == 3 || $data['mtype'] == 16 || $data['mtype'] == 28 || $$data['mtype'] == 30 || $data['mtype'] == 31)
			{
				$return['msg'] = "Die Schilde k�nnen aufgrund des Nebels nicht aktiviert werden";
				return $return;
			}
			if ($data['traktormode'] == 1)
			{
			...
			if ($loadtime > 0)
			{
				...
				{
					$return['msg'] = "Die Schilde sind polarisiert und k�nnen fr�hestens um ".date("H:i",$loadtime+10800)." Uhr wieder aktiviert werden";
					return $return;
				}
	...
	function etransfer($shipId,$shipId2,$count,$userId,$mode)
	{
		...
		{
			$return['msg'] = "F�r den Energietransfer werden ".($this->cclass['crew_min'] - 2)." Crewmitglieder ben�tigt";
			return $return;
		}
		if ($this->cenergie == 0)
		{
	...
	function transferto($shipId,$shipId2,$goodId,$goodcount,$mode,$freq=0)
	{
		...
		{
			$return['msg'] = "Ein D�mpfungsfeld sch�tzt das Hangardeck, Transport fehlgeschlagen";
			return $return;
		}
		...
		else
		{
			...
			{
				$return['msg'] = "Das Tr�mmerfeld kann nicht erfasst werden";
				return $return;
			}
			if ($shipdata['cloak'] == 1)
			{
			...
			if ($this->db->query("SELECT id FROM stu_torpedo_types WHERE goods_id=".$goodId,1) != 0 && $shipdata['c']['id'] != 2)
			{
				...
				{
					$probes != 0 ? $return['msg'] = "Es k�nnen maximal "$return['msg']c]['torps']-$probes)." Torpedos geladen werden - ".$probes." Rampen durch Sonden belegt" $return['msg']g] = "Es k�nnen maxima$return['msg']a['c']['torps']." Torpedos geladen werden";
					return $return;
				}
			...
			if ($goodId == 35 || $goodId == 36 || $goodId == 37 || $goodId == 204)
			{
				...
				{
					$return['msg'] = "Es k�nnen maximal $return['msg']c]['probe_stor']." Sonden geladen werden";
					return $return;
				}
				...
				if ($probes >= $sonkap)
				{
					$return['msg'] = "Es k�nnen keine Sonden mehr geladen werden";
					return $return;
				}
		...
		if ($myUser->ulevel == 1)
		{
			$return['msg'] = "Als Level 1 Kolonist kannst Du die Warenb�rse nicht nutzen";
			return $return;
		}
		if ($this->user != 17)
		{
			if ($this->cclass['crew_min'] -2 > $this->ccrew)
			{
				$return['msg'] = "Zum beamen werden ".($this->cclass['crew_min'] - 2)." Crewmitglieder ben�tigt";
				return $return;
			}
			if ($this->ccrew < 2)
			{
				$return['msg'] = "Zum beamen werden 2 Crewmitglieder ben�tigt";
				return $return;
			}
		}
	...
	function transferfrom ($shipId,$shipId2,$goodId,$goodcount,$mode,$freq=0)
	{
		...
		if ($myUser->ulevel == 1)
		{
			$return['msg'] = "Kolonisten mit Level 1 k�nnen keinen Waren von anderen Schiffen/Kolonien transferieren";
			return $return;
		}
		if (($this->user != 19) && ($goodId == 301))
		{
		...
		if ($this->db->query("SELECT id FROM stu_torpedo_types WHERE goods_id=".$goodId,1) != 0)
		{
			...
			{
				$return['msg'] = "Es k�nnen maximal ".$this->cclass['torps']." Torpedos geladen werden";
				return $return;
			}
			...
			{
				$probes != 0 ? $return['msg'] = "Es k�nnen maximal ".($this->cclass['torps']-$probes)." Torpedos geladen werden - ".$probes." Rampen durch Sonden belegt" $return['msg']g] = "Es k�nnen maximal ".$this->cclass['torps']." Torpedos geladen werden";
				return $return;
			}
		...
		if ($goodId == 35 || $goodId == 36 || $goodId == 37 || $goodId == 204)
		{
			...
			{
				$return['msg'] = "Es k�nnen maximal ".$this->cclass['probe_stor']." Sonden geladen werden";
				return $return;
			}
	...
	function spawnship($classId,$mode,$spawnId)
	{
		...
		{
			$return['msg'] = "Es werden 100 Sympathie zum beantragen eines Schiffes ben�tigt - Vorhanden sind nur ".$myUser->usymp;
			return $return;
		}
	...
	function colonize($shipId,$colId)
	{
		...
		if ($this->user > 100)
		{
			...
			{
				$return['msg'] = "Zur Besiedlung dieses Plantentypes wird Kolonistenlevel 4 ben�tigt";
				return $return;
			}
			if ($myUser->ulevel < 6 && ($coldata['colonies_classes_id'] == 4 || $coldata['colonies_classes_id'] == 5 || $coldata['colonies_classes_id'] == 6))
			{
				$return['msg'] = "Zur Besiedlung dieses Plantentypes wird Kolonistenlevel 6 ben�tigt";
				return $return;
			}
			if ($myUser->ulevel < 8 && $coldata['colonies_classes_id'] > 6)
			{
				$return['msg'] = "Zur Besiedlung dieses Plantentypes wird Kolonistenlevel 8 ben�tigt";
				return $return;
			}
			...
			{
				$return['msg'] = "Es sind nur 2 Planeten der Klasse L,N,G,K,H oder X zus�tzlich zur Klasse M pro Kolonist erlaubt";
				return $return;
			}
		...
		if ($stor < 50)
		{
			$return['msg'] = "Zum Kolonisieren werden 50 Baumaterial an Bord des Schiffes ben�tigt - Vorhanden sind nur ".$stor;
			return $return;
		}
		if ($this->cenergie < 10)
		{
			$return['msg'] = "Zum Kolonisieren wird 10 Energie ben�tigt - Vorhanden ist nur ".$this->cenergie;
			return $return;
		}
	...
	function ebatt($shipId,$count,$userId)
	{
		...
		{
			$return['msg'] = "F�r das entladen der Ersatzbatterie werden "$return['msg']n] - 2)." Crewmitglieder ben�tigt";
			return $return;
		}
	...
	function bussard($shipId,$count,$userId)
	{
		...
		if ($data['crew_min'] - 2 > $data['crew'])
		{
			$return['msg'] = "F�r die Bussard-Kollektoren werden "$return['msg']n] - 2)." Crewmitglieder ben�tigt";
			return $return;
		}
		if ($data['energie'] == 0)
		{
			$return['msg'] = "Keine Energie f�r die Bussard-Kollektoren vorhanden";
			return $return;
		}
		global $myMap;
	...
	function phaser($shipId,$shipId2,$userId,$strbk=1,$ksscheck=1,$react=0,$df=1)
	{
		...
		{
			$this->msghandle("F�r einen Schu� werde$ship1['c']1['c']['crew_min']." Crewmitglieder ben�tigt");
			return $this->cret($destroy);
		}
		...
		{
			$this->msghandle("Es k�nnen keine Kolonisten unter Level 2 angegriffen werden");
			return $this->cret($destroy);
		}
		...
		if ($field['type'] == 16)
		{
			...
			if ($ship1['schilde_aktiv'] == 1 && $ship1['schilde'] > $selfschaden)
			{
				...
				$this->msghandle("Explosion durch entz�ndetes Metreongas - Schildschaden: ".$selfschaden.", Schilde jetzt bei ".$sschilde);
			...
			elseif ($ship1['schilde_aktiv'] == 1 && $ship1['schilde'] < $selfschaden)
			{
				...
				{
					$this->trumfield($shipId);
					$myHistory->addEvent("Die ".addslashes($ship1['name'])." wurde in Sektor ".$ship1['coords_x']."/".$ship1['coords_y'].($ship1['wese'] == 2 ? " (2)" : "")." durch eine Metreongasexplosion zerst�rt$ship1['name']d]);
					$this->msghandle("Explosion durch entz�ndetes Metreongas - Schilde brechen zusammen - H�llenbruch!<br>Das Schiff wurde zerst�rt");
					return $this->cret($destroy);
				}
				$this->db->query("UPDATE stu_ships SET schilde=0,schilde_aktiv=0,huelle=".$srhuelle." WHERE id=".$shipId);
				$this->msghandle("Explosion durch entz�ndetes Metreongas - Schilde brechen zusammen, H�lle jetzt bei ".$srhuelle);
			}
			else
			{
				...
				if ($srhuelle < 1)
				{
					$this->trumfield($shipId);
					$myHistory->addEvent("Die ".addslashes($ship1['name'])." wurde in Sektor ".$ship1['coords_x']."/".$ship1['coords_y'].($ship1['wese'] == 2 ? " (2)" : "")." durch eine Metreongasexplosion zerst�rt$ship1['name']d]);
					$this->msghandle("Explosion durch entz�ndetes Metreongas - H�llenbruch!<br>Das Schiff wurde zerst�rt");
					return $this->cret($destroy);
				}
				$this->db->query("UPDATE stu_ships SET huelle=".$srhuelle." WHERE id=".$shipId);
				$this->msghandle("Explosion durch entz�ndetes Metreongas - H�llenschaden: ".$selfschaden.", H�lle jetzt bei ".$srhuelle);
			}
		}
		...
		{
		// -------- Pulsphaser
			$this->msghandle("Die ".$ship1['name']." schie�t mit einer Pulswaffe auf die $ship1['name']e],3);
			for ($i=0;$i<3;$i++)
			{
				$sad = 0;
				$nr++;
				...
				{
					$this->msghandle("Die ".$ship2['name']." f�hrt ein Ausweichman�ver durch - Schuss ".$nr." hat sein Ziel verfehlt",3);
					continue;
				}
				if ($sa == 1 && $schilde > 0)
				{
				...
				if ($destroy == 1 && $ship2['c']['trumfield'] == 0)
				{
					...
					$this->msghandle("Schuss ".$nr." trifft - H�llenbruch!<br>Das Schiff wurde zerst�rt!",3);
					$myHistory->addEvent("Die ".addslashes($ship2['name'])." wurde in Sektor ".$ship2['coords_x']."/".$ship2['coords_y'].($ship2['wese'] == 2 ? " (2)" : "")." von der ".addslashes($ship1['name'])." zerst�rt$ship2['name']d],1);
					break;
				}
				if ($destroy == 1 && $ship2['c']['trumfield'] == 1)
				{
					$this->msghandle("Schuss ".$nr." trifft - Das Tr�mmerfeld wurde beseitigt");
				...
				if ($destroy == 0 && $ship2['c']['trumfield'] == 0)
				{
					...
					elseif ($sad == 1) $this->msghandle("Schuss ".$nr." trifft - Schilde brechen zusammen - H�lle bei ".$huelle,3);
					else $this->msghandle("Schuss ".$nr." trifft - H�lle bei ".$huelle,3);
				}
			}
			if ($destroy == 1)
			{
		...
		// -------- Energiedisruptor
			...
			{
				$this->msghandle("Die ".$ship2['name']." f�hrt ein Ausweichman�ver durch - Der Phaserschuss hat sein Ziel verfehlt",3);
				$miss = 1;
			}
			else
			{
				$this->msghandle("Die ".$ship1['name']." schie�t auf die $ship1['name']e]." - kein direkter Schaden",3);
				...
				{
					$this->deactivatevalue($ship2['id'],"cloak",$ship2['user_id']);
					$this->msghandle("Die Tarnung der ".$ship2['name']." f�llt aus",3);
		...
		// -------- Normal
			...
			if ($miss != 1)
			{
				...
				{
					$this->trumfield($shipId2);
					$this->msghandle("Die ".$ship1['name']." schie�t auf die $ship1['name']e]." - H�llenbruch!<br>Das Schiff wurde zerst�rt!",3);
					if ($ship2['c']['probe'] == 0) $myHistory->addEvent("Die ".addslashes($ship2['name'])." wurde in Sektor ".$ship2['coords_x']."/".$ship2['coords_y'].($ship2['wese'] == 2 ? " (2)" : "")." von der ".addslashes($ship1['name'])." zerst�rt$ship2['c']d],1);
					$this->returndestroy = 1;
					$myComm->sendpm($ship2['user_id'],$this->user,$this->pmmsg,2);
					return $this->cret($destroy);
				}
				if ($destroy == 1 && $ship2['c']['trumfield'] == 1)
				{
					$this->msghandle("Die ".$ship1['name']." schie�t im Sektor $ship1['name']x]."/$ship1['name']y]." auf das Tr�mmerfeld - Das Tr�mmerfeld wurde beseitigt",3);
					$this->deletetrumfield($shipId2);
					$this->returndestroy = 1;
					$myComm->sendpm($ship2['user_id'],$this->user,$this->pmmsg,2);
					return $this->cret($destroy);
				}
				if ($destroy == 0 && $ship2['c']['trumfield'] == 1) $this->msghandle("Die ".$ship1['name']." schie�t auf das Tr�mmerfeld - Status: ".$huelle,3);
				else
				{
					if ($schilde > 0 && $durch == 0 && $sa == 1 && $sad == 0) $this->msghandle("Die ".$ship1['name']." schie�t auf die $ship1['name']e]."<br>Die Schilde der $ship1['name']e]." sind bei ".$schilde,3);
					elseif ($durch == 1) $this->msghandle("Die ".$ship1['name']." schie�t auf die $ship1['name']e]."<br>Die H�lle de$ship1['name']ame]." ist bei ".$huelle,3);
					elseif ($sad == 1) $this->msghandle("Die ".$ship1['name']." schie�t auf die $ship1['name']e]."<br>Die Schilde der $ship1['name']e]." brechen zusammen<br>Die H�lle ist bei ".$huelle,3);
					else $this->msghandle("Die ".$ship1['name']." schie�t auf die $ship1['name']e]."<br>Die H�lle de$ship1['name']ame]." ist bei ".$huelle,3);
				}
				$this->db->query("UPDATE stu_ships SET huelle=".$huelle.",schilde=".$schilde.",schilde_aktiv=".$sa." WHERE id=".$shipId2);
	...
	function deletetrumfield($shipId)
	{
		if ($this->db->query("SELECT ships_rumps_id FROM stu_ships WHERE id=".$shipId,1) == 208)
		{
			...
			$myHistory->addEvent($this->db->query("SELECT name FROM stu_ships WHERE id=".$shipId,1)." wurde von ".addslashes($this->db->query("SELECT user FROM stu_user WHERE id=".$user,1))." ge�ffnet",$user,1);
			return 0;
		}
	...
	function strikeback($shipId,$shipId2,$userId,$msg=1,$breen=0,$sbtype=1)
	{
		...
		if ($nr == 1)
		{
			$return['msg'] .= "Die ".$target['name']." f�hrt keine Reaktion durch";
			return $return;
		}
		if (($target['ships_rumps_id'] == 211) && ($target['alertlevel'] != 1))
		{
			...
			elseif ($this->getcountbygoodid(308,$target['id']) == 1)
			{
				...
				$msg .= "J�ger wurde gestartet<br>";
			}
			else
			{
				$this->npclaunchfighter($target['id'],1);
				$this->npclaunchfighter($target['id'],1);
				$msg .= "J�gerstaffel wurde gestartet<br>";
			}
		}
	...
	function trumfield($shipId)
	{
		...
		if ($shipdata['ships_rumps_id'] == 123)
		{
			...
			for ($i=0;$i<mysql_num_rows($result);$i++)
			{
				...
				$myComm->sendpm($tmpdat['user_id'],"2","Die ".$tmpdat['name']." wurde in Sektor ".$tmpdat['coords_x']."/".$tmpdat['coords_y']." durch eine Subraumexplosion zerst�rt",2,1);
			}
		}
	...
	function transfercrew($shipId,$shipId2,$count,$mode,$way)
	{
		...
		if ($mode == "col")
		{
			if ($way == "to")
			{
				...
				{
					$return['msg'] = "Aufgrund von Wohnraummangel k�nnen keine Crewmitglieder transferiert werden";
					return $return;
				}
	...
	function traktor($shipId,$shipId2)
	{
		...
		if ($this->cclass['crew_min'] - 2 > $this->ccrew)
		{
			$return['msg'] = "Es werden ".($this->cclass['crew_min'] - 2)." Crewmitglieder ben�tigt";
			return $return;
		}
	...
	function shieldemitter($shipId,$count,$userId)
	{
		...
		{
			$return['msg'] = "Die Schilde sind bereits vollst�ndig geladen";
			return $return;
		}
		if ($data['c']['crew_min'] - 2 > $data['crew'] && $data['ships_rumps_id'] != 88)
		{
			$return['msg'] = "Es werden ".($data['c']['crew_min'] - 2)." Crewmitglieder ben�tigt";
			return $return;
		}
	...
	function torp($shipId,$shipId2,$userId,$type,$strbk=1,$react=0,$df=1)
	{
		...
		if ($ship1['c']['crew_min'] > $ship1['crew'])
		{
			$this->msghandle("Es werden ".$class['crew_min']." Crewmitglieder ben�tigt");
			return $this->cret($destroy);
		}
		if ($ship1['dock'] > 0)
		{
		...
		{
			$this->msghandle("Dieses Tr�mmerfeld kann mit den Sensoren nicht erfasst werden");
			return $this->cret($destroy);
		}
		if ($ship2['ships_rumps_id'] == 154)
		{
		...
		{
			$this->msghandle("Mit Level 1 kannst Du keine Torpedos abschie�en");
			return $this->cret($destroy);
		}
		global $myColony;
		...
		if ($type != 13 && $type != 14)
		{
			...
			else
			{
				...
				{
					$this->msghandle("Die ".$ship2['name']." f�hrt ein Ausweichman�ver durch - Der Torpedo hat sein Ziel verfehlt",3);
					$miss = 1;
				}
				else
				{
					if ($type == 12 && rand(1,5) == 1)
					{
						...
						$myHistory->addevent("Subrauminstabilit�t in Sektor $ship2['coords_x']x]."/$ship2['coords_x']y]$ship2['coords_x']e] == 2 ? " (2)" : ""$ship2['coords_x']d],1);
						...
						while($sd=mysql_fetch_assoc($sr))
						{
							...
							if ($sd['huelle'] <= $dmg)
							{
								$myComm->sendpm($sd['user_id'],2,"Die ".$sd['name']." wurde aufgrund einer Subraumschockwelle in Sektor ".$ship2['coords_x']."/".$ship2['coords_y']." zerst�rt",2);
								$myHistory->addEvent("Die ".addslashes($sd['name'])." wurde in Sektor ".$ship2['coords_x']."/".$ship2['coords_y'].($ship2['wese'] == 2 ? " (2)" : "")." durch eine Subraumschockwelle zerst�rt$sd['name']d],1);
								$this->trumfield($sd['id']);
							}
							else
							{
								$myComm->sendpm($sd['user_id'],2,"Die ".$sd['name']." wurde aufgrund einer Subraumschockwelle in Sektor ".$ship2['coords_x']."/".$ship2['coords_y']." besch�digt - H�lle bei$sd['user_id']lle]-$dmg),2);
		...
		elseif ($schilde <= 0 && $sad == 0 && $ship2['schilde_aktiv'] == 1 && $ship2['c']['trumfield'] == 0 && $destroy != 1) $this->msghandle("Die Schilde der ".$ship2['name']." brechen zusammen - H�lle bei ".$huelle,3);
		elseif ($destroy == 1 && $ship2['c']['trumfield'] == 0) $this->msghandle("H�llenbruch!<br>Die $ship2['c']e]." wurde zerst�rt!",3);
		elseif ($ship2['c']['trumfield'] == 1 && $destroy == 0) $this->msghandle("Status des Tr�mmerfelds: ".$huelle,3);
		elseif ($ship2['c']['trumfield'] == 1 && $destroy == 1) $this->msghandle("Das Tr�mmerfeld wurde beseitigt",3);
		elseif ($miss != 1) $this->msghandle("Die H�lle der $ship2['name']e]." ist bei ".$huelle,3);
	...
	function alertlevel($shipId,$level,$userId)
	{
		...
		if (($data['c']['crew_min'] - 2 > $data['crew']) && ($data['ships_rumps_id'] != 88))
		{
			$return['msg'] = "Zum �ndern der Alarmstufe werden "$return['msg']c]['crew_min'] - 2)." Crewmitglieder ben�tigt";
			return $return;
		}
		global $myUser;
		...
		$return['msg'] = "<table bgcolor=#262323><tr><td class=tdmainobg><img src=".$this->gfx."/buttons/alert".$level.".gif></td>
					<td class=tdmainobg align=center><img src=".$this->gfx."/ships/".$mpf.$data['ships_rumps_id'].".gif></td>
					<td class=tdmainobg>Alarmstufe wurde ge�ndert</td></tr></table>";
		return $return;
	}
	...
	function collect($shipId,$count,$userId)
	{
		...
		if ($data['c']['crew_min'] - 2 > $data['crew'])
		{
			$return['msg'] = "F�r die Erz-Kollektoren werden "$return['msg']c]['crew_min'] - 2)." Crewmitglieder ben�tigt";
			return $return;
		}
		if ($data['schilde_aktiv'] == 1)
		{
		...
		if ($data['energie'] < 1)
		{
			$return['msg'] = "Keine Energie f�r die Erz-Kollektoren vorhanden";
			return $return;
		}
		global $myMap;
		if ($count == "max") $count = $data['energie'];
	...
	function selfdestruct($shipId,$code)
	{
		...
		{
			$return['msg'] = "Falscher Best�tigungscode!";
			return $return;
		}
		global $myUser,$myHistory;
		$this->trumfield($shipId);
		if ($myUser->ulevel > 1) $myHistory->addEvent("Die ".addslashes($this->cname)." hat sich im Sektor ".$this->ccoords_x."/".$this->ccoords_y.($this->cwese == 2 ? " (2)" : "")." selbst zerst�rt",$this->user,2);
		$this->db->query("DELETE FROM stu_ships_action WHERE mode='destruct' AND ships_id=".$shipId);
		$return['msg'] = "Selbstzerst�rung erfolgreich";
		return $return;
	}
	...
	function loadwarpcore($shipId,$max=0)
	{
		...
		{
			$return['msg'] = "Zum laden des Warpkerns werden ".($this->cclass['crew_min'] - 2)." Crewmitglieder ben�tigt";
			return $return;
		}
		...
		if ($this->creaktormodlvl == 105)
		{
			...
			if ($deut < 6)
			{
				$return['msg'] = "Es wird 6 Deuterium ben�tigt";
				return $return;
			}
			$plas = $this->getcountbygoodid(15,$shipId);
			if ($plas < 6)
			{
				$return['msg'] = "Es wird 6 Plasma ben�tigt";
				return $return;
			}
			if ($max == 1)
			{
		...
		else
		{
			...
			if ($dil == 0)
			{
				$return['msg'] = "Es wird 1 Dilithium ben�tigt";
				return $return;
			}
			$am = $this->getcountbygoodid(5,$shipId);
			if ($am < 2)
			{
				$return['msg'] = "Es werden 2 Antimaterie ben�tigt";
				return $return;
			}
			$deut = $this->getcountbygoodid(2,$shipId);
			if ($deut < 2)
			{
				$return['msg'] = "Es werden 2 Deuterium ben�tigt";
				return $return;
			}
			if ($max == 1)
			{
	...
	function dock($shipId,$shipId2,$userId)
	{
		...
		if ($data2['schilde_aktiv'] == 1)
		{
			$return['msg'] = "Andocken nicht m�glich - Die $return['msg']e]." hat die Schilde aktiviert";
			return $return;
		}
		...
		if ($data['c']['crew_min'] - 2 > $data['crew'])
		{
			$return['msg'] = "Zum andocken werden mindestens ".($data['c']['crew_min'] - 2)." Crewmitglieder ben�tigt";
			return $return;
		}
		if ($this->cdock != 0)
		{
		...
		{
			$return['msg'] = "Alle Dockpl�tze der ".stripslashe$return['msg']e])." sind belegt";
			return $return;
		}
	...
	function undock($shipId,$shipId2,$userId)
	{
		...
		if ($this->cclass['crew_min'] - 2 > $this->ccrew)
		{
			$return['msg'] = "Zum Abdocken werden ".($this->cclass['crew_min'] -2)." Crewmitglieder ben�tigt";
			return $return;
		}
	...
	function attackcolfield($shipId,$colId,$mode,$fieldId,$freq=0)
	{
		...
		{
			$return['msg'] = "Es k�nnen keine Kolonisten unter Level 2 angegriffen werden";
			return $return;
		}
		if ($this->cenergie == 0)
		{
		...
		{
			$return['msg'] = "Es werden ".$this->cclass['crew_min']." Crewmitglieder ben�tigt";
			return $return;
		}
		...
		{
			$return['msg'] = "Dieses Geb�ude kann nicht angegriffen werden";
			return $return;
		}
		...
		{
			$return['msg'] = "Der Phaserschu� wurde von der Atmosph�re kompensiert";
		...
		$cl[$fieldId] == 1 ? $msg = "Die ".$this->cname." schie�t auf der Kolonie $coldata['name']e]." auf das Geb�ude auf Feld ".($fieldId+1) : $msg = "Die ".$this->cname." schie�t auf der Kolo$coldata['name']['name']." auf das Geb�$coldata['name']['name']." auf Feld ".($fieldId+1);
		...
		{
			$return['msg'] .= $msg."<br>Der Schu� richtete keinen Schaden an";
			$defend = $myColony->defendColony($colId,$shipId);
			if ($defend['msg'] != "")
			{
				$return['msg'] .= "<br><strong>Kolonieverteidigung</strong><br>".$defend['msg'];
		...
		if ($fielddata['integrity'] - $schaden <= 0)
		{
			...
			$msg .= "<br>Das Geb�ude wurde zerst�rt - Es ga$building['bev_use']use]." Tote";
		...
		else
		{
			...
			$msg .= "<br>Schaden: ".$schaden." - Integrit�t: "$fielddata['integrity']y]-$schaden);
		}
		$this->cenergie -= 1;
		$return['msg'] = $msg;
		$defend = $myColony->defendColony($colId,$shipId);
	...
	function npclaunchfighter($basis,$type=1)
	{
		...
		{
			$return['msg'] = "Keine startbereiten J�ger mehr vorhanden";
			return $return;
		}
		$data = $this->getclassbyid($classId);
		$huell = $this->getmodulebyid($huellmod);
		...
		$name = addslashes($basedata['name']." J�ger");
		$nextid = $this->db->query("SELECT max(id) AS next_id FROM stu_ships",4);
		$nextid = $nextid['next_id'] + 1;
		...
		$return['msg'] = "J�ger wurde gestartet";
		return $return;
	}
	...
	function npcreturnfighter($basis,$type=1)
	{
		...
		$return['msg'] = $anzahl." J�ger sind gelandet";
		return $return;
	}
	...
	function wormhole($shipId,$userId,$fleet=0)
	{
		...
		{
			$return['msg'] = "Es werden ".($data['c']['crew_min'] - 1)." Crewmitglieder ben�tigt um in das Wurmloch zu fliegen";
			if ($fleet > 0)
			{
				$this->db->query("UPDATE stu_ships SET fleets_id=0 WHERE id=".$shipId);
				$return['msg'] .= "<br>Die ".$data['name']." hat sich von der Flotte gel�st";
			}
			return $return;
		}
		if ($data['traktormode'] == 1)
		...
		if ($data['energie'] < 13)
		{
			$return['msg'] = "Es wird mindestens 13 Energie ben�tigt um in das Wurmloch zu fliegen";
			return $return;
		}
		if ($data['dock'] > 0)
		{
		...
		if ($wormdata['stable'] == 0)
		{
			...
			if (rand(1,3) == 1)
			{
				...
				$this->trumfield($shipId);
				$return['msg'] = $msg."Die ".$data['name']." wurde im instabilen Wurmloch zerst�rt";
				return $return;
			}
		}
	...
	function buildkonstrukt($shipId)
	{
		...
		if ($this->cships_rumps_id != 7)
		{
			$return['msg'] = "Zum Konstruktbau wird ein Workbee ben�tigt";
			return $return;
		}
		...
		if ($this->cclass['crew_min'] > $this->ccrew)
		{
			$return['msg'] = "Zum Konstruktbau werden ".$this->cclass['crew_min']." Crewmitglieder ben�tigt";
			return $return;
		}
		global $myMap;
		...
		if ($this->cenergie < 15)
		{
			$return['msg'] = "Es wird 15 Energie auf dem Workbee ben�tigt - Vorhanden sind nur ".$this->cenergie;
			return $return;
		}
	...
	function buildstation($huellmod,$schildmod,$waffenmod,$sensormod,$reaktormod,$epsmod,$computermod,$stationId,$shipId)
	{
		...
		{
			$return['msg'] = "Um eine Station bauen zu k�nnen muss ein Workbee am Konstrukt angedockt sein";
			return $return;
		}
		global $myColony;
		...
		{
			$return['msg'] = "Es wird ".$builddat['eps_cost']." Energie ben�tigt - Vorhanden ist nur ".$this->cenergie;
			return $return;
		}
		...
		{
			$return['msg'] = "Es sind ".round($wirtsum,2)." Punkte vorhanden - Zum Bau werden ".round($wirtdata+$points+$buildwp,2)." ben�tigt";
			return $return;
		}
		if ($this->cenergie - $builddat['eps_cost'] < 0)
		{
			$return['msg'] = "Es wird ".$this->cclass['eps_cost']." Energie ben�tigt - Vorhanden ist aber nur ".$this->cenergie;
			return $return;
		}
	...
	function mincost($classId,$userId,$shipId)
	{
		...
		for ($i=0;$i<count($result);$i++)
		{
			...
			{
				$return['msg'] = "Es werden ".$result[$i]['count']." ".$result[$i]['name']." ben�tigt - Vorhanden sind nur ".$rress;
				$return['code'] = 0;
				return $return;
			}
		}
	...
	function adddockperm($shipId,$type,$dockId,$perm)
	{
		...
		if ($type == "shipid")
		{
			...
			{
				$return['msg'] = "F�r dieses Schiff ist bereits eine Andockregel vorhanden";
				return $return;
			}
			$this->db->query("INSERT INTO stu_dock_permissions (ships_id,type,id2,mode) VALUES ('".$shipId."','ship',".$dockId.",'".$perm."')");
			$return['msg'] = "Die Andockerlaubnis f�r die ".stripslashe$return['msg']e])." wurde erstellt";
		}
		elseif ($type == "allyid")
		{
			global $myAlly;
			...
			{
				$return['msg'] = "F�r diese Allianz ist bereits eine Andockregel vorhanden";
				return $return;
			}
			$this->db->query("INSERT INTO stu_dock_permissions (ships_id,type,id2,mode) VALUES ('".$shipId."','ally',".$dockId.",'".$perm."')");
			$return['msg'] = "Die Andockerlaubnis f�r die Allianz $return['msg']e]." wurde erstellt";
		}
		elseif ($type == "userid")
		{
			global $myUser;
			...
			{
				$return['msg'] = "F�r diesen Siedler ist bereits eine Andockregel vorhanden";
				return $return;
			}
			$this->db->query("INSERT INTO stu_dock_permissions (ships_id,type,id2,mode) VALUES ('".$shipId."','user',".$dockId.",'".$perm."')");
			$return['msg'] = "Die Andockerlaubnis f�r den User $return['msg']r]." wurde erstellt";
		}
		return $return;
	}
	...
	function deldockpermission($shipId,$dockId)
	{
		...
		$return['msg'] = "Die Andockerlaubnis wurde gel�scht";
		return $return;
	}
	...
	function randomPhaser($shipId)
	{
		...
		if ($this->cenergie == 0)
		{
			$return['msg'] = "Zum Abschu� einer Phasersalve wird mindestens 1 Energie ben�tigt";
			return $return;
		}
	...
	function checkModules($huellmod,$schildmod,$waffenmod,$epsmod,$computermod,$sensormod,$reaktormod,$classId,$shipId,$userId)
	{
		...
		if ($huellmod == 0)
		{
			$return['msg'] = "Es wurde kein H�llenmodul ausgew�hlt";
			$return['code'] = 0;
			return $return;
		}
		$module = $this->getmodulebyid($huellmod);
		...
		{
			$return['msg'] = "Dieses H�llenmodul kann in den gew�hlten Schiffsrumpf nicht eingebaut werden";
			$return['code'] = 0;
			return $return;
		}
		...
		if ($stor == 0)
		{
			$return['msg'] = "Dieses H�llenmodul befindet sich nicht an Bord des Konstrukts";
			$return['code'] = 0;
			return $return;
		}
		if ($schildmod == 0)
		{
			$return['msg'] = "Es wurde kein Schildmodul ausgew�hlt";
			$return['code'] = 0;
			return $return;
		}
		$module = $this->getmodulebyid($schildmod);
		...
		{
			$return['msg'] = "Dieses Schildmodul kann in den gew�hlten Schiffsrumpf nicht eingebaut werden";
			$return['code'] = 0;
			return $return;
		}
		...
		if ($waffenmod != 0)
		{
			...
			{
				$return['msg'] = "Dieses Waffenmodul kann in den gew�hlten Schiffsrumpf nicht eingebaut werden";
				$return['code'] = 0;
				return $return;
			}
		...
		if ($epsmod == 0)
		{
			$return['msg'] = "Es wurde kein EPS-Gittermodul ausgew�hlt";
			$return['code'] = 0;
			return $return;
		}
		$module = $this->getmodulebyid($epsmod);
		...
		{
			$return['msg'] = "Dieses EPS-Gittermodul kann in den gew�hlten Schiffsrumpf nicht eingebaut werden";
			$return['code'] = 0;
			return $return;
		}
		...
		if ($computermod == 0)
		{
			$return['msg'] = "Es wurde kein Computermodul ausgew�hlt";
			$return['code'] = 0;
			return $return;
		}
		$module = $this->getmodulebyid($computermod);
		...
		{
			$return['msg'] = "Dieses Computermodul kann in den gew�hlten Schiffsrumpf nicht eingebaut werden";
			$return['code'] = 0;
			return $return;
		}
		...
		if ($sensormod == 0)
		{
			$return['msg'] = "Es wurde kein Sensorenmodul ausgew�hlt";
			$return['code'] = 0;
			return $return;
		}
		$module = $this->getmodulebyid($sensormod);
		...
		{
			$return['msg'] = "Dieses Sensorenmodul kann in den gew�hlten Schiffsrumpf nicht eingebaut werden";
			$return['code'] = 0;
			return $return;
		}
		...
		if ($reaktormod != 0)
		{
			...
			{
				$return['msg'] = "Dieses Reaktormodul kann in den gew�hlten Schiffsrumpf nicht eingebaut werden";
				$return['code'] = 0;
				return $return;
			}
			$stor = $this->checkstoragebygoodid(1,$module['goods_id'],$shipId);
	...
	function defendship($shipId2,$shipId,$userId)
	{
		...
		if ($this->cclass['crew_min'] > $this->ccrew)
		{
			$return['msg'] = "Zur Verteidigung werden ".$this->cclass['crew_min']." Crewmitglieder ben�tigt";
			return $return;
		}
	...
	function fireprobe($shipId,$classlvl)
	{
		...
		if ($classlvl == 5)
		{
			...
			for ($i=1;$i<=4;$i++)
			{
				...
				else $msg .= "<br>Drohne konnte Ziel nicht aufsp�ren";
	...
	function npcchangefiremode($shipId)
	{
		...
		if ($class['size'] < 5)
		{
			$return['msg'] = "Umstellung nicht m�glich";
			return $return;
		}
		$beg = 0;
	...
	function redalert($x,$y,$shipId=0,$fleet=0,$wese=1,$userId=0)
	{
		...
		while($data=mysql_fetch_assoc($result))
		{
			...
			if ($res['code'] == 1)
			{
				...
				$myComm->sendpm($data['user_id'],2,"<strong>Kampf in Sektor ".$x."/".$y."</strong><br>Die ".stripslashes($data['name'])." (Alarm Rot/Freund-Feind-Erkennung) beschie�t die ".stripslashe$data['user_id']e])." des Users ".stripslashes($myUser->getfield("user",$userId))." (".$this->user.")",2);
			}
			$this->returnmsg = "";
			$this->pmmsg = "";
	...
	function gravitonscan($shipId)
	{
		...
		if ($this->cenergie < 8)
		{
			$return['msg'] = "F�r einen Gravitonscan werden 8 Energie ben�tigt";
			return $return;
		}
		...
		if ($cmodule['lvl'] < 3)
		{
			$return['msg'] = "F�r einen Gravitonscan wird mindestens ein Isolinearer Computer ben�tigt";
			return $return;
		}
		$chance = round($smodule['lvl'] * $data['c']['sensormod'] * $cmodule['lvl'] / 2);
	...
	function shiprepair ($shipId,$targetId)
	{
		...
		{
			$return['msg'] = "Wracks k�nnen nicht repariert werden";
			return $return;
		}
		if ($targetdata['c']['id'] == 111)
		{
			$return['msg'] = "Konstrukte k�nnen nicht repariert werden";
			return $return;
		}
		...
		{
			$return['msg'] = "Das Schiff ist nicht besch�digt";
			return $return;
		}
		...
		if ($targetdata['c']['slots'] == 0)
		{
			...
			{
				$return['msg'] = "Zur Reparatur wird ".$cost[0]." Energie ben�tigt - Vorhanden ist nur ".$this->cenergie;
				return $return;
			}
			$count = $this->getcountbygoodid(3,$this->cid);
			if ($cost[3] > $count)
			{
				$return['msg'] = "Zur Reparatur werden ".$cost[3]." Baumaterial ben�tigt - Vorhanden sind nur ".$count;
				return $return;
			}
			$count = $this->getcountbygoodid(6,$this->cid);
			if ($cost[6] > $count)
			{
				$return['msg'] = "Zur Reparatur werden ".$cost[6]." Duranium ben�tigt - Vorhanden sind nur ".$count;
				return $return;
			}
			$count = $this->getcountbygoodid(9,$this->cid);
			if ($cost[9] > $count)
			{
				$return['msg'] = "Zur Reparatur werden ".$cost[9]." Tritanium ben�tigt - Vorhanden sind nur ".$count;
				return $return;
			}
			$count = $this->getcountbygoodid(10,$this->cid);
			if ($cost[10] > $count)
			{
				$return['msg'] = "Zur Reparatur werden ".$cost[10]." Iso-Chips ben�tigt - Vorhanden sind nur ".$count;
				return $return;
			}
			$count = $this->getcountbygoodid(12,$this->cid);
			if ($cost[12] > $count)
			{
				$return['msg'] = "Zur Reparatur werden ".$cost[12]." Kelbonit ben�tigt - Vorhanden sind nur ".$count;
				return $return;
			}
			$count = $this->getcountbygoodid(14,$this->cid);
			if ($cost[14] > $count)
			{
				$return['msg'] = "Zur Reparatur werden ".$cost[14]." Nitrium ben�tigt - Vorhanden sind nur ".$count;
				return $return;
			}
			$count = $this->getcountbygoodid(15,$this->cid);
			if ($cost[15] > $count)
			{
				$return['msg'] = "Zur Reparatur werden ".$cost[15]." Plasma ben�tigt - Vorhanden sind nur ".$count;
				return $return;
			}
			$count = $this->getcountbygoodid(19,$this->cid);
			if ($cost[19] > $count)
			{
				$return['msg'] = "Zur Reparatur werden ".$cost[19]." Gel-Packs ben�tigt - Vorhanden sind nur ".$count;
				return $return;
			}
			$count = $this->getcountbygoodid($cost['modules']['huelleg'],$this->cid);
			if ($cost['modules']['huellec'] > $count)
			{
				$return['msg'] = "Zur Reparatur werden ".$cost['modules']['huellec']." H�llenmodule ben�tigt - Vorhanden sind nur ".$count;
				return $return;
			}
			$count = $this->getcountbygoodid($cost['modules']['schildeg'],$this->cid);
			if ($cost['modules']['schildec'] > $count)
			{
				$return['msg'] = "Zur Reparatur werden ".$cost['modules']['schildec']." Schildmodule ben�tigt - Vorhanden sind nur ".$count;
				return $return;
			}
			$count = $this->getcountbygoodid($cost['modules']['antriebg'],$this->cid);
			if ($cost['modules']['antriebc'] > $count)
			{
				$return['msg'] = "Zur Reparatur werden ".$cost['modules']['antriebc']." Antriebsmodule ben�tigt - Vorhanden sind nur ".$count;
				return $return;
			}
			if ($shipdata['reaktormodlvl'] > 0)
			{
				...
				{
					$return['msg'] = "Zur Reparatur werden ".$cost['modules']['reaktorc']." Reaktormodule ben�tigt - Vorhanden sind nur ".$count;
					return $return;
				}
			}
			...
			{
				$return['msg'] = "Zur Reparatur werden ".$cost['modules']['sensorc']." Sensormodule ben�tigt - Vorhanden sind nur ".$count;
				return $return;
			}
			$count = $this->getcountbygoodid($cost['modules']['computerg'],$this->cid);
			if ($cost['modules']['computerc'] > $count)
			{
				$return['msg'] = "Zur Reparatur werden ".$cost['modules']['computerc']." Computermodule ben�tigt - Vorhanden sind nur ".$count;
				return $return;
			}
			if ($shipdata['waffenmodlvl'] > 0)
			{
				...
				{
					$return['msg'] = "Zur Reparatur werden ".$cost['modules']['waffenc']." Waffenmodule ben�tigt - Vorhanden sind nur ".$count;
					return $return;
				}
			}
			...
			{
				$return['msg'] = "Zur Reparatur werden ".$cost['modules']['epsc']." EPS-Gittermodule ben�tigt - Vorhanden sind nur ".$count;
				return $return;
			}
		...
		else
		{
			...
			{
				$return['msg'] = "Zur Reparatur wird ".$cost[0]." Energie ben�tigt - Vorhanden ist nur ".($this->cenergie $return['msg']e]);
				return $return;
			}
	...
	function loadstationbatt($id,$count)
	{
		...
		{
			$return['msg'] = "Die Reservebatterie der ".$shipdata['name']." ist bereits vollst�ndig aufgeladen";
			return $return;
		}
		if ($shipdata['slots'] == 0)
		{
			$return['msg'] = "Nur Stationen k�nnen mit Reparaturschiffen aufgeladen werden";
			return $return;
		}
	...
	function dabo($val)
	{
		...
		{
			$return['msg'] = "Nicht gen�gend Latinum vorhanden";
			return $return;
		}
		if ($val < 1 || $val > 40)
		{
