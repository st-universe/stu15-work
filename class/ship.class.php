...
class ship
{
	...
	function move($shipId,$x,$y,$userId,$fleetmove="",$red=1)
	{
		...
		if ($cd[race] == 15 && $data[cloak] == 1)
		{
			if (($this->db->query("SELECT count(id) FROM stu_ships WHERE coords_x BETWEEN ($x-1) AND ($x+1) AND coords_y BETWEEN ($y-1) AND ($y+1) AND ships_rumps_id=189",1) != 0) AND ($this->db->query("SELECT id FROM stu_ships_action WHERE ships_id = ".$data[id]." AND mode='borderdet' AND ships_id2 = 15",1) == 0))
			{
				...
				if ($userId > 100) $myComm->sendpm($cd[race],2,"Ann�herungsalarm: Die ".$data[name]." von ".$myUser->uuser." (".$data[user_id].") wurde bei ".$x."/".$y." entdeckt",2);
			}
		}
		...
		if ($huelle < $data[huelle] && $huelle > 0) $msg .= "<br>H�llenschaden: ".($data[huelle]-$huelle)." - H�lle bei ".$huelle;
		if ($huelle < 1)
		{
			$this->trumfield($shipId);
			$destroy = 1;
	...
	function transferto($shipId,$shipId2,$goodId,$goodcount,$mode,$freq=0)
	{
		...
		else
		{
			...
			if ($this->db->query("SELECT id FROM stu_torpedo_types WHERE goods_id=".$goodId,1) != 0 && $shipdata[c][id] != 2)
			{
				...
				{
					$probes != 0 ? $return[msg] = "Es k�nnen maximal ".($shipdata[c][torps]-$probes)." Torpedos geladen werden - ".$probes." Rampen durch Sonden belegt" : $return[msg] = "Es k�nnen maximal ".$shipdata[c][torps]." Torpedos geladen werden";
					return $return;
				}
			...
			if ($goodId == 35 || $goodId == 36 || $goodId == 37 || $goodId == 204)
			{
				...
				{
					$return[msg] = "Es k�nnen maximal ".$shipdata[c][probe_stor]." Sonden geladen werden";
					return $return;
				}
	...
	function transferfrom ($shipId,$shipId2,$goodId,$goodcount,$mode,$freq=0)
	{
		...
		if ($this->db->query("SELECT id FROM stu_torpedo_types WHERE goods_id=".$goodId,1) != 0)
		{
			...
			{
				$probes != 0 ? $return[msg] = "Es k�nnen maximal ".($this->cclass[torps]-$probes)." Torpedos geladen werden - ".$probes." Rampen durch Sonden belegt" : $return[msg] = "Es k�nnen maximal ".$this->cclass[torps]." Torpedos geladen werden";
				return $return;
			}
	...
	function ebatt($shipId,$count,$userId)
	{
		...
		{
			$return[msg] = "F�r das entladen der Ersatzbatterie werden ".($data[crew_min] - 2)." Crewmitglieder ben�tigt";
			return $return;
		}
	...
	function bussard($shipId,$count,$userId)
	{
		...
		if ($data[crew_min] - 2 > $data[crew])
		{
			$return[msg] = "F�r die Bussard-Kollektoren werden ".($data[crew_min] - 2)." Crewmitglieder ben�tigt";
			return $return;
		}
		if ($data[energie] == 0)
		{
	...
	function phaser($shipId,$shipId2,$userId,$strbk=1,$ksscheck=1,$react=0,$df=1)
	{
		...
		{
			$this->msghandle("F�r einen Schu� werden ".$ship1[c][crew_min]." Crewmitglieder ben�tigt");
			return $this->cret($destroy);
		}
		...
		if ($field[type] == 16)
		{
			...
			elseif ($ship1[schilde_aktiv] == 1 && $ship1[schilde] < $selfschaden)
			{
				...
				{
					$this->trumfield($shipId);
					$myHistory->addEvent("Die ".addslashes($ship1[name])." wurde in Sektor ".$ship1[coords_x]."/".$ship1[coords_y].($ship1[wese] == 2 ? " (2)" : "")." durch eine Metreongasexplosion zerst�rt",$ship1[user_id]);
		...
		{
		// -------- Pulsphaser
			$this->msghandle("Die ".$ship1[name]." schie�t mit einer Pulswaffe auf die ".$ship2[name],3);
			for ($i=0;$i<3;$i++)
			{
				$sad = 0;
				$nr++;
				...
				if ($destroy == 1 && $ship2[c][trumfield] == 0)
				{
					...
					$myHistory->addEvent("Die ".addslashes($ship2[name])." wurde in Sektor ".$ship2[coords_x]."/".$ship2[coords_y].($ship2[wese] == 2 ? " (2)" : "")." von der ".addslashes($ship1[name])." zerst�rt",$ship2[user_id],1);
					break;
				}
		...
		// -------- Energiedisruptor
			...
			{
				$this->msghandle("Die ".$ship1[name]." schie�t auf die ".$ship2[name]." - kein direkter Schaden",3);
		...
		// -------- Normal
			...
			if ($miss != 1)
			{
				...
				{
					$this->trumfield($shipId2);
					$this->msghandle("Die ".$ship1[name]." schie�t auf die ".$ship2[name]." - H�llenbruch!<br>Das Schiff wurde zerst�rt!",3);
					if ($ship2[c][probe] == 0) $myHistory->addEvent("Die ".addslashes($ship2[name])." wurde in Sektor ".$ship2[coords_x]."/".$ship2[coords_y].($ship2[wese] == 2 ? " (2)" : "")." von der ".addslashes($ship1[name])." zerst�rt",$ship2[user_id],1);
					$this->returndestroy = 1;
					$myComm->sendpm($ship2[user_id],$this->user,$this->pmmsg,2);
					return $this->cret($destroy);
				}
				if ($destroy == 1 && $ship2[c][trumfield] == 1)
				{
					$this->msghandle("Die ".$ship1[name]." schie�t im Sektor ".$ship1[coords_x]."/".$ship1[coords_y]." auf das Tr�mmerfeld - Das Tr�mmerfeld wurde beseitigt",3);
					$this->deletetrumfield($shipId2);
					$this->returndestroy = 1;
					$myComm->sendpm($ship2[user_id],$this->user,$this->pmmsg,2);
					return $this->cret($destroy);
				...
				{
					if ($schilde > 0 && $durch == 0 && $sa == 1 && $sad == 0) $this->msghandle("Die ".$ship1[name]." schie�t auf die ".$ship2[name]."<br>Die Schilde der ".$ship2[name]." sind bei ".$schilde,3);
					elseif ($durch == 1) $this->msghandle("Die ".$ship1[name]." schie�t auf die ".$ship2[name]."<br>Die H�lle der ".$ship2[name]." ist bei ".$huelle,3);
					elseif ($sad == 1) $this->msghandle("Die ".$ship1[name]." schie�t auf die ".$ship2[name]."<br>Die Schilde der ".$ship2[name]." brechen zusammen<br>Die H�lle ist bei ".$huelle,3);
					else $this->msghandle("Die ".$ship1[name]." schie�t auf die ".$ship2[name]."<br>Die H�lle der ".$ship2[name]." ist bei ".$huelle,3);
				}
				$this->db->query("UPDATE stu_ships SET huelle=".$huelle.",schilde=".$schilde.",schilde_aktiv=".$sa." WHERE id=".$shipId2);
	...
	function torp($shipId,$shipId2,$userId,$type,$strbk=1,$react=0,$df=1)
	{
			...
			else
			{
				...
				else
				{
					if ($type == 12 && rand(1,5) == 1)
					{
						...
						$myHistory->addevent("Subrauminstabilit�t in Sektor ".$ship2[coords_x]."/".$ship2[coords_y].($ship2[wese] == 2 ? " (2)" : ""),$ship1[user_id],1);
						...
						while($sd=mysql_fetch_assoc($sr))
						{
							...
							if ($sd[huelle] <= $dmg)
							{
								...
								$myHistory->addEvent("Die ".addslashes($sd[name])." wurde in Sektor ".$ship2[coords_x]."/".$ship2[coords_y].($ship2[wese] == 2 ? " (2)" : "")." durch eine Subraumschockwelle zerst�rt",$ship2[user_id],1);
								$this->trumfield($sd[id]);
							}
							else
							{
								$myComm->sendpm($sd[user_id],2,"Die ".$sd[name]." wurde aufgrund einer Subraumschockwelle in Sektor ".$ship2[coords_x]."/".$ship2[coords_y]." besch�digt - H�lle bei ".($sd[huelle]-$dmg),2);
		...
		elseif ($destroy == 1 && $ship2[c][trumfield] == 0) $this->msghandle("H�llenbruch!<br>Die ".$ship2[name]." wurde zerst�rt!",3);
		...
		elseif ($miss != 1) $this->msghandle("Die H�lle der ".$ship2[name]." ist bei ".$huelle,3);
	...
	function alertlevel($shipId,$level,$userId)
	{
		...
		if (($data[c][crew_min] - 2 > $data[crew]) && ($data[ships_rumps_id] != 88))
		{
			$return[msg] = "Zum �ndern der Alarmstufe werden ".($data[c][crew_min] - 2)." Crewmitglieder ben�tigt";
			return $return;
		}
		global $myUser;
	...
	function collect($shipId,$count,$userId)
	{
		...
		if ($data[c][crew_min] - 2 > $data[crew])
		{
			$return[msg] = "F�r die Erz-Kollektoren werden ".($data[c][crew_min] - 2)." Crewmitglieder ben�tigt";
			return $return;
		}
		if ($data[schilde_aktiv] == 1)
		{
	...
	function dock($shipId,$shipId2,$userId)
	{
		...
		if ($data2[schilde_aktiv] == 1)
		{
			$return[msg] = "Andocken nicht m�glich - Die ".$data2[name]." hat die Schilde aktiviert";
			return $return;
		}
		...
		{
			$return[msg] = "Alle Dockpl�tze der ".stripslashes($data2[name])." sind belegt";
			return $return;
		}
	...
	function attackcolfield($shipId,$colId,$mode,$fieldId,$freq=0)
	{
		...
		$cl[$fieldId] == 1 ? $msg = "Die ".$this->cname." schie�t auf der Kolonie ".$coldata[name]." auf das Geb�ude auf Feld ".($fieldId+1) : $msg = "Die ".$this->cname." schie�t auf der Kolonie ".$coldata[name]." auf das Geb�ude ".$building[name]." auf Feld ".($fieldId+1);
		...
		if ($fielddata[integrity] - $schaden <= 0)
		{
			...
			$msg .= "<br>Das Geb�ude wurde zerst�rt - Es gab ".$building[bev_use]." Tote";
		...
		else
		{
			...
			$msg .= "<br>Schaden: ".$schaden." - Integrit�t: ".($fielddata[integrity]-$schaden);
		}
		$this->cenergie -= 1;
		$return[msg] = $msg;
		$defend = $myColony->defendColony($colId,$shipId);
	...
	function adddockperm($shipId,$type,$dockId,$perm)
	{
		...
		if ($type == "shipid")
		{
			...
			$return[msg] = "Die Andockerlaubnis f�r die ".stripslashes($data2[name])." wurde erstellt";
		}
		elseif ($type == "allyid")
		{
			global $myAlly;
			...
			$return[msg] = "Die Andockerlaubnis f�r die Allianz ".$data2[name]." wurde erstellt";
		}
		elseif ($type == "userid")
		{
			global $myUser;
			...
			$return[msg] = "Die Andockerlaubnis f�r den User ".$data2[user]." wurde erstellt";
		}
		return $return;
	}
	...
	function redalert($x,$y,$shipId=0,$fleet=0,$wese=1,$userId=0)
	{
		...
		while($data=mysql_fetch_assoc($result))
		{
			...
			if ($res[code] == 1)
			{
				...
				$myComm->sendpm($data[user_id],2,"<strong>Kampf in Sektor ".$x."/".$y."</strong><br>Die ".stripslashes($data[name])." (Alarm Rot/Freund-Feind-Erkennung) beschie�t die ".stripslashes($target[name])." des Users ".stripslashes($myUser->getfield("user",$userId))." (".$this->user.")",2);
			}
			$this->returnmsg = "";
			$this->pmmsg = "";
	...
	function shiprepair ($shipId,$targetId)
	{
		...
		else
		{
			...
			{
				$return[msg] = "Zur Reparatur wird ".$cost[0]." Energie ben�tigt - Vorhanden ist nur ".($this->cenergie + $targetdata[energie]);
				return $return;
			}
