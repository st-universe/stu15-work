...
class comm
{
	...
	function getknbylz($mark)
	{
		return $this->db->query("SELECT a.*,UNIX_TIMESTAMP(a.date) as date_tsp,b.user,b.status,b.rasse,b.picture from stu_kn_messages as a LEFT OUTER JOIN stu_user as b ON a.user_id=b.id WHERE a.id<=" . $mark . " ORDER BY a.date DESC LIMIT 5");
	}

	function getknbylzoff($mark)
	{
		return $this->db->query("SELECT a.*,UNIX_TIMESTAMP(a.date) as date_tsp,b.user,b.status,b.rasse,b.picture from stu_kn_messages as a LEFT OUTER JOIN stu_user as b ON a.user_id=b.id WHERE a.id<=" . $mark . " AND a.official=1 ORDER BY a.date DESC LIMIT 5");
	}

	function getknmaxid()
	{
		return $this->db->query("SELECT max(id) FROM stu_kn_messages", 1);
	}
	...
	function addkn($subject, $message, $off = 1)
	{
		$this->db->query("INSERT INTO stu_kn_messages (user_id,subject,text,date,official) VALUES ('" . $this->user . "','" . addslashes($subject) . "','" . addslashes($message) . "',NOW(),'" . $off . "')");
	}

	function getpms($cat)
	{
		return $this->db->query("SELECT a.id,a.sender,a.message,a.new,UNIX_TIMESTAMP(a.date) as date_tsp,b.user,b.rasse,b.status FROM stu_pms as a LEFT OUTER JOIN stu_user as b ON a.sender=b.id WHERE a.recipient='" . $this->user . "' AND a.cate=" . $cat . " AND a.recip_del=0 ORDER BY a.date DESC LIMIT 0,10");
	}

	function getpmsmark($begin, $cat = 1)
	{
		return $this->db->query("SELECT a.id,a.sender,a.message,a.new,UNIX_TIMESTAMP(a.date) as date_tsp,b.user,b.rasse,b.status FROM stu_pms as a LEFT OUTER JOIN stu_user as b ON a.sender=b.id WHERE a.recipient='" . $this->user . "' AND a.cate=" . $cat . " AND a.recip_del=0 ORDER BY a.date DESC LIMIT " . $begin . ",10");
	}
	...
	function markasread($pmId)
	{
		$this->db->query("UPDATE stu_pms SET new=0 WHERE id='" . $pmId . "'");
	}

	function checknewmsg()
	{
		return $this->db->query("SELECT COUNT(id) from stu_pms WHERE new='1' AND recipient=" . $this->user, 1);
	}
	...
	{
		$this->db->query("UPDATE stu_pms SET new=0 WHERE recipient='" . $this->user . "' AND cate=" . $cat);
	}

	function delallpms($cat)
	{
		$this->db->query("UPDATE stu_pms SET recip_del=1,new=0 WHERE cate=" . $cat . " AND recipient=" . $this->user);
	}
	...
	function getallylz()
	{
		return $this->db->query("SELECT kn_allylz from stu_user WHERE id='" . $this->user . "'", 1);
	}
	...
	{
		return $this->db->query("SELECT recipient,behaviour FROM stu_contactlist WHERE user_id='" . $this->user . "' ORDER BY recipient", 2);
	}
	...
	function getpmsbyuserid($userId)
	{
		return $this->db->query("SELECT sender,recipient,message,UNIX_TIMESTAMP(date) as date_tsp FROM stu_pms WHERE recipient=" . $userId . " OR sender=" . $userId . " ORDER BY date DESC", 2);
	}
	...
	function getknmsgbyid($msgId)
	{
		return $this->db->query("SELECT a.id,a.user_id,a.subject,a.text,UNIX_TIMESTAMP(a.date) as date_tsp,b.user,b.rasse,b.status,b.picture FROM stu_kn_messages as a LEFT OUTER JOIN stu_user as b ON a.user_id=b.id WHERE a.id=" . $msgId, 4);
	}
	...
	function checkcontact($user, $user2)
	{
		return $this->db->query("SELECT behaviour FROM stu_contactlist WHERE recipient=" . $user . " ANd user_id=" . $user2, 1);
	}
	...
	{
		$this->db->query("UPDATE stu_pms SET new=0 WHERE recipient=" . $this->user . " AND new=1", $this->dblink);
	}
	...
	function getoutpms()
	{
		return $this->db->query("SELECT a.id,a.message,a.recipient,UNIX_TIMESTAMP(a.date) as date_tsp,b.user FROM stu_pms as a LEFT OUTER JOIN stu_user as b ON a.recipient=b.id WHERE a.sender='" . $this->user . "' AND a.send_del=0 ORDER BY a.date DESC LIMIT 0,10", 2);
	}

	function getoutpmsmark($begin)
	{
		return $this->db->query("SELECT a.id,a.message,a.recipient,UNIX_TIMESTAMP(a.date) as date_tsp,b.user FROM stu_pms as a LEFT OUTER JOIN stu_user as b ON a.recipient=b.id WHERE a.sender='" . $this->user . "' AND a.send_del=0 ORDER BY a.date DESC LIMIT " . $begin . ",10", 2);
	}

	function delalloutpms()
	{
		$this->db->query("UPDATE stu_pms SET send_del=1 WHERE sender=" . $this->user);
	}

	function my_2d_sort(&$array, $sort, $d = 1)
	{
		usort($array, create_function('$a,$b', 'return strcasecmp($a["' . $sort . '"],$b["' . $sort . '"])* ' . $d . ';'));
	}
	...
	{
		return $this->db->query("SELECT COUNT(id) FROM stu_pms WHERE recipient=" . $this->user . " ANd new=1", 1);
	}

	function markcatasread($catId)
	{
		$this->db->query("UPDATE stu_pms SET new=0 WHERE cate=" . $catId . " ANd recipient=" . $this->user . " ANd new=1");
	}

	function getmaindeskpms()
	{
		return $this->db->query("SELECT LEFT(a.message,150) as msg,UNIX_TIMESTAMP(a.date) as date_tsp,b.user FROM stu_pms as a LEFT JOIN stu_user as b ON a.sender=b.id WHERE a.recipient=" . $this->user . " AND a.new=1 AND a.cate=1 ORDER BY a.id DESC LIMIT 5");
	}
	...
	function getknmsgbyuser($userId)
	{
		return $this->db->query("SELECT a.id,a.user_id,a.subject,a.text,UNIX_TIMESTAMP(a.date) as date_tsp,b.user,b.status,b.rasse,b.picture FROM stu_kn_messages as a LEFT OUTER JOIN stu_user as b ON a.user_id=b.id WHERE a.user_id=" . $userId . " ORDER BY a.date DESC");
	}
}