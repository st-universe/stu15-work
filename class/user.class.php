...
class user
{
	...
	{
		return $this->db->query("SELECT *,UNIX_TIMESTAMP(lastaction) as last_tsp from stu_user WHERE id='" . $userId . "'", 4);
	}

	function getMainUserByID($userId)
	{
		return $this->db->query("SELECT grafik,mozilla,lastloginround from stu_user WHERE id='" . $userId . "'", 4);
	}

	function updateUserById($userId, $value, $field)
	{
		$this->db->query("UPDATE stu_user SET " . $field . "='" . str_replace("\"", "", $value) . "' WHERE id='" . $userId . "'");
	}
...
Content-Type: text/html");
		...
		$return['code'] = 1;
		$return['msg'] = "Der Account wurde angelegt.<br>Du erh�lst in K�rze eine Email mit einem Aktivierungscode.<br><br>
		Viel Spa�";
		return $return;
	}
	...
	function setlevel()
	{
		...
		if ($data == 0) {
			$return['msg'] = "Du hast bereits das h�chste Level (8)";
			return $return;
		}
		if ($this->ulevel == 0) {
		...
		if ($this->usymp < $data['symp']) {
			$return['msg'] = "Es wird " . $data['symp'] . " Sympathie f�r Level " . $data['level'] . " ben�tigt - Vorhanden ist nur " . $this->usymp;
			return $return;
		}
	...
	{
		$this->db->query("UPDATE stu_user SET lastloginround=" . $round . " WHERE id=" . $this->uid);
	}
	...
	{
		return $this->db->query("SELECT id FROM stu_user WHERE id>100", 4);
	}
	...
	function passrecovery($userId)
	{
		...
		Achtung! Dieser Code ist 24 Stunden lang g�ltig<br><br>
		Mit freundlichen Gr��en<br><br>
		Das STU-Team", "From: Star Trek Universe <wolverine@stuniverse.de>
Content-Type: text/html");
		$return['msg'] = "Du erh�lst in wenigen Augenblicken eine Email mit einem RecoveryCode";
		return $return;
	}
	...
	function editpass($code, $pass, $pass2)
	{
		...
		if ($pass != $pass2) {
			$return['msg'] = "Die Passw�rter stimmen nicht �berein";
			return $return;
		}
		...
		$return['msg'] = "Das Passwort wurde erfolgreich ge�ndert";
		return $return;
	}

	function getonlineusercount()
	{
		return $this->db->query("SELECT count(id) FROM stu_user WHERE UNIX_TIMESTAMP(lastaction)>=" . (time() - 300) . " AND id>100", 1);
	}
	...
	{
		return $this->db->query("SELECT * FROM stu_user_profiles WHERE user_id=" . $userId, 4);
	}

	function updateProfileById($userId, $value, $field)
	{
		$this->db->query("UPDATE stu_user_profiles SET " . $field . "='" . $value . "' WHERE user_id=" . $userId);
	}
	...
	function getfield($value, $userId)
	{
		return $this->db->query("SELECT " . $value . " FROM stu_user WHERE id=" . $userId, 1);
	}
	...
	function dvm()
	{
		$this->db->query("UPDATE stu_user SET vac=0,vactime=0 WHERE id=" . $this->user);
	}
}