...
if (!$section) {
	...
	if (mysql_num_rows($result) == 0) echo "<tr><td class=tdmainobg align=center>Keine Schiffe vorhanden</td></td></tr>";
		...
		while ($tmp = mysql_fetch_assoc($result)) {
			...
			if ($data['crew'] > 0 || $data['c']['probe'] == 1 || $data['c']['id'] == 88) {
				if ($data['reaktormodlvl'] > 0) {
					if ($data['warpcore'] < $data['verbrauch']) {
						...
						if ($dv > 0) {
							$rd = 0;
							$rda = " (<font color=Yellow>" . @floor($dv / ($data['verbrauch'] + $data['erzeugung'])) . "</font>)";
						} else $rd = 0;
			...
			if ($fleet['ships_id'] == $data['id']) $ftd = "<a href=?page=ship&action=delfleet&id=" . $data['id'] . " onMouseOver=\"cp('df" . $i . "','buttons/fl_flag3')\" onMouseOut=\"cp('df" . $i . "','buttons/fl_flag2')\"><img src=" . $grafik . "/buttons/fl_flag2.gif name=df" . $i . " border=0 title='Flotte aufl�sen'></a>";
			...
			else $ftd = "<a href=?page=ship&action=newfleet&id=" . $data['id'] . " onMouseOver=\"cp('nf" . $i . "','buttons/fl_flag2')\" onMouseOut=\"cp('nf" . $i . "','buttons/fl_flag1')\"><img src=" . $grafik . "/buttons/fl_flag1.gif name=nf" . $i . " border=0 title='Flotte gr�nden'></a>";
			$data['lss'] == 1 ? $l = "*" : $l = "-";
			$data['kss'] == 1 ? $k = "*" : $k = "-";
	...
	if ($fleet != 0 && $user == $fleet['user_id']) {
		if ($action == "phaser" && $id && $id2) {
			$result = $myFleet->phaser($id, $id2);
			$fleetac = 1;
		}
		if ($action == "torp" && $id && $id2) {
			$result = $myFleet->torp($id, $id2);
			$fleetac = 1;
		}
		if ($action == "move" && $id && $x > 0 && $y > 0) {
			$result = $myFleet->move($id, $x, $y);
			$fleetac = 1;
		}
		if ($action == "movewh" && $id) {
			$result = $myFleet->wormhole($id);
			$fleetac = 1;
		}
		if ($action == "ap" && $id && $way && $fields && is_numeric($fields)) {
			$result = $myFleet->autopilot($id, $way, $fields);
			$fleetac = 1;
		}
		if ($command == "shieldson" && $id) {
			$result = $myFleet->activatevalue($id, "schilde_aktiv");
			$fleetac = 1;
		}
		if ($command == "agreen" && $id) {
			$result = $myFleet->alertlevel($id, 1, $user);
			$fleetac = 1;
		}
		if ($command == "ayellow" && $id) {
			$result = $myFleet->alertlevel($id, 2, $user);
			$fleetac = 1;
		}
		if ($command == "ared" && $id) {
			$result = $myFleet->alertlevel($id, 3, $user);
			$fleetac = 1;
		}
		if ($command == "bussard" && $id && $ener_count) {
			$result = $myFleet->bussard($id, $ener_count);
			$fleetac = 1;
		}
		if ($command == "erz" && $id && $ener_count) {
			$result = $myFleet->collect($id, $ener_count);
			$fleetac = 1;
		}
		if ($command == "replikatoron" && $id) {
			$result = $myFleet->activatevalue($id, "replikator");
			$fleetac = 1;
		}
		if ($command == "ebatt" && $id && $batt_count) {
			$result = $myFleet->ebatt($id, $batt_count);
			$fleetac = 1;
		}
		if ($command == "shload" && $id && $load) {
			$result = $myFleet->shload($id, $load);
			$fleetac = 1;
		}
		if ($command == "ksson" && $id) {
			$result = $myFleet->activatevalue($id, "kss");
			$fleetac = 1;
		}
		if ($command == "cloakon" && $id) {
			$result = $myFleet->activatevalue($id, "cloak");
			$fleetac = 1;
		}
		if ($command == "decloak" && $id) {
			$result = $myFleet->decloak($id);
			$fleetac = 1;
		}
		if ($command == "fmphaser" && $id) {
			$result = $myFleet->fmphaser($id);
			$fleetac = 1;
		}
		if ($command == "fmtorp" && $id) {
			$result = $myFleet->fmtorp($id);
			$fleetac = 1;
		}
	}
	...
	if ($myShip->cclass['erz'] > 0 && ($myShip->ctype == 4 || $myShip->ctype == 5 || $myShip->ctype == 17 || $myShip->ctype == 18 || $myShip->ctype == 19 || $myShip->ctype == 20 || $myShip->ctype == 32)) {
		if ($myShip->ctype == 4) {
			$vk = 4;
			$t = "Iridium";
		} elseif ($myShip->ctype == 5) {
			$vk = 6;
			$t = "Iridium";
		} elseif ($myShip->ctype == 17) {
			$vk = 2;
			$t = "Kelbonit";
		} elseif ($myShip->ctype == 18) {
			$vk = 3;
			$t = "Kelbonit";
		} elseif ($myShip->ctype == 19) {
			$vk = 2;
			$t = "Nitrium";
		} elseif ($myShip->ctype == 20) {
			$vk = 3;
			$t = "Nitrium";
		}
	...
	if ($myShip->cships_rumps_id == 211) {
		$lfighter = "<a href=main.php?page=ship&section=showship&action=npclfighter&id=" . $id . " onMouseOver=\"cp('npclf','buttons/lfighter2')\" onMouseOut=\"cp('npclf','buttons/lfighter1')\"><img src=" . $grafik . "/buttons/lfighter1.gif name=npclf border=0> J�ger starten</a>";
		$cfighter = "<a href=main.php?page=ship&section=showship&action=npccfighter&id=" . $id . " onMouseOver=\"cp('npccf','buttons/cfighter2')\" onMouseOut=\"cp('npccf','buttons/cfighter1')\"><img src=" . $grafik . "/buttons/cfighter1.gif name=npccf border=0> R�ckruf</a>";
	...
	if ($myShip->cclass['probe'] != 1) echo "<tr>
		...
		<tr>
			...
			<option value=1" . $gruen . ">Gr�n</option>
			<option value=2" . $gelb . ">Gelb</option>
			<option value=3" . $rot . ">Rot</option>
			</select> <input type=submit value='�ndern' class=button></td>
		</tr></form>
		<form action=main.php method=post>
		<input type=hidden name=page value=ship>
		<input type=hidden name=section value=showship>
	...
	if ($fleet['ships_id'] == $id) {
		...
		<option value=agreen>Alarmstufe: Gr�n</option>
		...
		</select> <input type=submit value=Ausf�hren class=button></td>
		</tr></form>";
	...
	if ($stationinfo != 0 && $stationinfo['id'] != $id && $myShip->cclass['probe'] != 1) {
		...
		if (($stationinfo['ships_rumps_id'] == 87 || $stationinfo['ships_rumps_id'] == 100) && $stationinfo['user_id'] == 14) {
			echo "<td class=tdmainobg><a href=main.php?page=ship&section=fergtrade&id2=" . $stationinfo['id'] . "&id=" . $id . " onMouseOver=\"cp('fergtrade','buttons/fergtrade2')\" onMouseOut=\"cp('fergtrade','buttons/fergtrade1')\"><img src=" . $grafik . "/buttons/fergtrade1.gif name=fergtrade border=0 title='Handel'></a></td>
			  ...
			  <td class=tdmainobg><a href=main.php?page=ship&section=techtrade&id2=" . $stationinfo['id'] . "&id=" . $id . " onMouseOver=\"cp('techtrade','buttons/forsch2')\" onMouseOut=\"cp('techtrade','buttons/forsch1')\"><img src=" . $grafik . "/buttons/forsch1.gif name=techtrade border=0 title='Datenb�rse'></a></td>";
		}
		echo "</tr></table>";
	}
	echo "<br>";
	...
	if ($myShip->cships_rumps_id != 111) {
		if ($myShip->ckss == 1) {
			...
			/*echo "<tr><td class=tdmainobg><img src=gfx/gesch.gif></td>
				<td class=tdmainobg>Ein frohes Weihnachtsfest w�nscht euch euer STU-Team</td>
	...
	<td valign=top class=tdmainobg>
		...
		<tr>
			...
			<td class=tdmainobg>Name <input class=text type=text size=20 name=new_name value=\"" . htmlspecialchars($myShip->cname) . "\"> <input class=button type=submit value=�ndern></td>
			</form>
		</tr>
		<tr>
			<td class=tdmainobg><a href=main.php?page=ship&section=selfdestruct&id=" . $id . "><font color=#ff0000>Selbstzerst�rung</font></a></td>
		</tr>
		</table>
	</td>
	</tr>
	...
	if ($lrsdata == -2) echo "<tr><td class=tdmainobg align=center colspan=6>Nicht gen�gend Energie vorhanden</td></tr>";
		...
		while ($data = mysql_fetch_assoc($lrsdata)) {
			...
			if ($data['cloak'] == 0 || ($myUser->ually == $myUser->getfield("allys_id", $data['user_id']) && $myUser->ually != 0)) {
				...
				echo "<td class=tdmainobg align=center>" . $lrspic . "</td>
					...
					<td class=tdmainobg align=center>";
				if ($data['cloak'] == 0) echo $data['energie'];
				echo "</td>
					<td class=tdmainobg align=center>";
				if ($data['cloak'] == 0) echo $lrsdatacrew;
				echo "</td>
					<td class=tdmainobg align=center>";
				if ($data['cloak'] == 0) echo $ws;
				echo "</td>
					<td class=tdmainobg align=center>";
				if ($data['cloak'] == 0) echo $schilde;
				echo "</td>
				</tr>";
			} else $cloaked++;
		}
	...
	if ($myUser->ulevel < 2) {
		...
		</tr><tr><td class=tdmainobg>Diese funktion steht erst ab Level 2 zur Verf�gung</td></tr>
		</table>";
		exit;
	}
	$mode == "col" ? $shipdata2 = $myColony->getcolonybyid($id2) : $shipdata2 = $myShip->getdatabyid($id2);
	if ($shipdata2 == 0) $txt = "Schiff nicht vorhanden";
	if ($myShip->ccoords_x != $shipdata2['coords_x'] || $myShip->ccoords_y != $shipdata2['coords_y'] || $myShip->cwese != $shipdata2['wese']) $txt = "Beide Schiffe m�ssen sich im selben Sektor befinden";
	if ($myShip->cenergie < 2 && $myShip->cclass['probe'] == 0) $txt = "Zum Scannen werden 2 Energie ben�tigt";
	if ($myShip->cenergie < 1 && $myShip->cclass['probe'] == 1) $txt = "Zum Scannen wird 1 Energie ben�tigt";
	if ($myShip->ccrew < 1 && $myShip->cclass['probe'] == 0) $txt = "Zum Scannen wird mindests 1 Crewmitglied ben�tigt";
	if ($txt) echo "<table bgcolor=#262323 cellspacing=1 cellpadding=1><tr><td class=tdmain><strong>Meldung</strong></td></tr><tr><td class=tdmainobg align=center>" . $txt . "</td></tr></table>";
		...
		if ($myShip->cclass['probe'] == 1) {
			$myShip->lowerfield("energie", 1, $id, $user);
			$myShip->cenergie -= 1;
		} else {
			$myShip->lowerfield("energie", 2, $id, $user);
			$myShip->cenergie -= 2;
		}
		...
		elseif ($shipdata2['user_id'] == 30) $pplicon = "<img src=" . $grafik . "/buttons/ppl_karemma.gif title='Karemma'> ";
			...
			if ($udata['rasse'] == 1) $pplicon = "<img src=" . $grafik . "/buttons/ppl_fed.gif title='F�deration'> ";
	...
	echo "<table width=100% bgcolor=#262323 cellspacing=1 cellpadding=1>
	<tr>
	<td width=100% class=tdmain>/ <a href=?page=ship>Schiffe</a> / <a href=?page=ship&section=showship&id=" . $id . ">" . $myShip->cname . "</a> / <strong>Selbstzerst�rung</strong></td>
	</tr>
	</table><br>
	...
	<input type=hidden name=id value=" . $id . ">
		<td class=tdmainobg>Bitte diesen Code in das Feld eingeben um die Selbstzerst�rung zu best�tigen:<br><strong>" . $code . "</strong><br><input class=text type=text size=6 maxlength=6 name=destructcode> <input class=button type=submit value=Best�tigung></td>
	</tr>
	</form>
	</table>";
	...
	echo "<table width=400 bgcolor=#262323 cellspacing=1 cellpadding=1><tr>
		<td></td>
		<td class=tdmainobg align=center><strong>Verf�gbar</strong></td>
	...
	echo "<table width=100% bgcolor=#262323 cellspacing=1 cellpadding=1>
	<tr>
	<td width=100% class=tdmain>/ <a href=?page=ship>Schiffe</a> / <a href=?page=ship&section=showship&id=" . $id . ">" . $myShip->cname . "</a> / <strong>Datenb�rse</strong></td>
	</tr>
	</table><br>";
	if ($result) echo "<table bgcolor=#262323 cellspacing=1 cellpadding=1><tr><td class=tdmain><strong>Meldung</strong></td></tr><tr><td class=tdmainobg>" . $result['msg'] . "</td></tr></table><br>";
	...
	<tr>
		<td class=tdmainobg width=500>
		Hast Du Informationen �ber die Kolonie ID: <input type=text size=5 name=shipid class=text>? <input type=submit value=Erfragen class=button>
	</tr>
	</form>
	<form action=main.php method=post>
	<input type=hidden name=page value=ship>
	...
	<td class=tdmain align=Center>Schiffsreparatur: " . stripslashes($targetdata['name']) . " (H�lle: " . $targetdata['huelle'] . "/" . $targetdata['maxhuell'] . ")</td></tr>
	<tr>
	...
	</table><br><br>Diese Kolonie befindet sich in einer Pufferzone, die zwischen den M�chten des Ivor-Vertrages und der <font color=#B54A29><b>Kzinti Hegemonie</b></font> vereinbart wurde. In einem von allen Beteiligten Vertrag wurde die Kolonisierung dieser Zone untersagt.<br><br>Mit der Kolonisierung fortfahren und den Vertrag brechen?";
	...
	if (!is_numeric($val)) {
		...
		<tr><td class=tdmainobg>Zahl zwischen 1 und 40 w�hlen <input type=text size=2 class=text maxlength=2 name=val value=" . $oval . "><br>
